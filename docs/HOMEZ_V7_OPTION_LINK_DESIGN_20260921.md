...
> **후속(2026-09-21 2차)**: 이 문서의 1차 프로토타입에 식별자 이중 경로·등록 결과 연결·서버 확정 준비 판정·콘솔 화면을 더해
> 채택 후보를 완성했다 — **최신 상태·패치는 `docs/HOMEZ_V7_OPTION_LINK_ADOPTION_20260921.md`를 본다.** `docs/proposals/`의 패치 파일은 그 2차
> 패치로 교체됐다(아래 §7의 "12개 파일 +1,958" 기술은 1차 기준). 화면 미구현·`attach_coupang_identifiers` 미배선이라는 아래 기술도 2차에서 해소됐다.
>
