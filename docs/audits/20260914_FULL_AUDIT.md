...
### 13.7 Phase 7 — 개발본/설치본 DB 선택 준비(읽기 전용)

지시문 Phase 7은 "결정하지 않는다, 준비 문서만 작성한다"를 명시했다.
`docs/audits/20260915_DEV_VS_INSTALLED_DB_DECISION_PREP.md`를 새로
작성해, 두 DB의 현재 상태(적용 Migration 수, 테이블 수, 핵심 테이블
존재 여부 — 전부 읽기 전용 재확인, 2026-09-15 기준)와 선택지 3가지
(개발본 유지 / 설치본 최신화 / 재설치)의 실행 경로·장단점을
정리했다. 결론은 "결정하지 않는다" — 설치본에 보존해야 할 실제
데이터가 있는지가 이 문서가 확인하지 않은 핵심 미지수이며, 사용자
답변 없이는 B/C 중 어느 쪽도 구체화할 수 없다.

이번 세션이 추가한 Migration 4개(Phase 2~5, `20260915_00`~`03`)를
포함해 두 실제 DB 파일 중 어느 쪽에도 여전히 아무것도 적용하지
...
### 13.9 Phase 9 — 안전 기능 11개 구현(HOMEZ_USER_OPERATION_SETTINGS.md
7-8/7-11/7-16/8-5/8-6/8-16/8-19/10-4/10-5/10-17/10-18)

11개 항목을 전수 조사했다(상세 근거·수치는
`docs/audits/20260915_PHASE9_SAFETY_FEATURES_SCOPING.md` 참고). 결과:
**8-16만 이번 라운드에서 구현 완료**, 7-8은 기존에 이미 부분
구현(단일 엔드포인트 10초 간격만) 상태 그대로 두었고, 나머지 9개는
전부 미구현으로 확인됐다.

**8-16("매입처 조회 실패가 계속되면 사용자에게 알린다") 구현**:
`PurchaseChannelConnection.consecutive_failure_count`(신규 Migration
`20260915_04`)를 추가해 실제 조회 실패마다(실패 종류 무관) 증가·
성공 시 0으로 리셋한다. 임계치(3회, 문서에 구체적 수치가 없어
추측하지 않고 조정 가능한 상수로 둠)에 "처음" 도달하는 순간에만
활성 SUPER_ADMIN 전원에게 알림(`SUPPLIER_LOOKUP_REPEATED_FAILURE`
신규 이벤트)을 보낸다. 회귀 과정에서 idempotency_key 충돌로 두
번째 실패 스트릭의 알림이 조용히 스킵되는 실제 결함을 발견·수정
했고(알림 발생 시각을 키에 포함), 새 Migration이 `purchase_channel_
connections` 원본 생성 Migration의 DDL-diff 테스트를 다시 깨뜨리면서
(Phase 3과 동일한 패턴) 컬럼-스트립 정규식이 `NOT NULL` 제약을
처리하지 못하는 별도의 잠재 버그도 함께 드러나 두 파일 모두
수정했다.

**나머지 9개를 미착수로 남긴 이유**: 각각 "게이트 하나 추가"가
아니라 독립적인 신규 기능이며, 상당수가 이 저장소에 아직 없는
인프라를 전제한다 — 캐싱 계층(`app/domains/cache/`는 완전히 빈
스텁), 일간/주기 실행 스케줄러(현재 등록된 작업은 주간 백업
리허설 1개뿐), 외부 회수·판매중지 정보 출처(전혀 미정 — 추측으로
연동을 만들지 않는다는 이 감사 원칙에 정면으로 어긋남),
`ProductCandidate` 모델의 비교 대상 필드 자체 부재 등. 이런 항목을
한 라운드에 몰아서 구현하면 충분히 검증되지 않은 안전 기능이
오작동할 위험이 커지는데, 이는 정확히 이 감사가 막으려는 것과
같은 종류의 문제다. 각 항목의 구체적 선행 조건(무엇이 없어서 못
만들었는지, 다음에 먼저 정할 것)을 스코핑 문서에 정리해 다음
라운드가 재조사 없이 바로 시작할 수 있게 했다.

**테스트 결과**: 서비스 레벨 신규 5건, Migration 신규 4건,
...
### 13.11 Phase 9A~9J — 안전 기능 10개 구현(HOMEZ_USER_OPERATION_SETTINGS.md
7-8/7-11/7-16/8-5/8-6/8-19/10-4/10-5/10-17/10-18)

13.9 시점에 8-16만 구현하고 미착수로 남겨뒀던 10개 항목을 이번
라운드에서 전부 착수했다. 순서대로 요약한다(각 항목의 정확한 판정과
근거는 `docs/HOMEZ_USER_OPERATION_SETTINGS_AUDIT_20260909.md`의
해당 행이 원본이다).

**Phase 9A(7-8, 호출제한·캐시)** — `PurchaseChannelConnection.
rate_limited_until`(429 시 자동 백오프)와 `PriceStockQuoteCache`
(회사·연결·상품·옵션별 격리, 가격/재고 독립 TTL)를 신설하고
`lookup_product_cached()`를 캐시 진입점으로 추가했다. 발주 직전
조회(`order_submission_service.py`)는 여전히 이 캐시를 쓰지 않는다
(가격 인상을 놓치지 않기 위한 기존 설계 원칙을 그대로 지킴).

**Phase 9B(7-11, 매입처 사건 반복 시 자동일시중지)** —
`SupplierIncidentService`가 품절/오배송/취소/배송지연/인증실패
5종을 회사·연결별로 기록하고, 회사별로 설정 가능한 기간·건수
임계치를 넘으면 **그 연결의 발주 기능만** PAUSED로 낮춘다(다른
연결·다른 기능·회사 전체 `FunctionAutomationState`에는 영향 없음).
자동 재활성화는 없다 — 관리자가 확인 사유를 남겨야만 해제된다.

**Phase 9C(7-16, 휴면 공급처 재점검)** — `last_successful_order_at`
기준으로 휴면 연결을 감지해 감사 이벤트로 남기지만, 독립 차단
게이트로 승격하지는 않았다(기존 `verified_at` 최신성 검사에 정보만
더하는 수준) — **PARTIALLY_IMPLEMENTED로 남겨둔 이유가 이것**이다.

**Phase 9D/9E(8-5/8-6, 가격·재고 TTL)** — `PriceCacheTtlSetting`
(기본 30분)·`StockCacheTtlSetting`(기본 10분)을 회사별로 독립
설정 가능하게 추가했다. Fake-clock 테스트로 "가격은 유효한데
재고만 만료" 같은 경계 케이스까지 확인했다.

**Phase 9F(8-19, 가상재고 0 제안)** — `VirtualStockZeroProposal`이
조회 실패·미지원·판매가능 확인불가 3가지 경로에서 자동 생성되고
SUPER_ADMIN에게 통지한다. **실제 판매채널에는 아무것도 자동
반영하지 않는다** — 제안일 뿐이며, 승인/거부는 항상 사람이 하고
재고 확인이 나중에 성공해도 자동으로 해소되지 않는다.

**Phase 9G(10-4, 상품 속성 불일치 차단)** — 신규 도메인
`app/domains/product_attribute_match`가 상품명/옵션/수량/사이즈/
제조사/원산지 6개 필드를 매입처·판매채널·HOMEZ 현재 값 3소스로
정규화 비교한다. **문자열 유사도는 쓰지 않는다** — 정규화(공백
접기+대소문자 무시) 후 완전 일치만 MATCHED, 소스가 하나뿐이면
UNCONFIRMED(비교 불가를 일치로 오판하지 않음). 실제 발주
(`order_submission_service.py`)와 실제 쿠팡 전송
(`listing_wizard_live_service.py::preflight()`) 양쪽에 차단
게이트로 배선했다. **다만 매입처 어댑터 계약
(`ChannelProductOption`/`ProductLookupResult`)이 제조사·원산지·
사이즈·수량 필드를 아예 갖고 있지 않아**, 상품명 외 5개 필드는
비교기는 준비됐지만 실제 매입처 값을 자동으로 채워 넣는 호출부가
아직 없다(수동 API로 값을 넣는 것은 가능) — 어댑터 계약 확장은
실제 온채널 API 응답 필드 확인이 선행돼야 해 이번 라운드에
포함하지 않았다.

**Phase 9H(10-5, 비교 UI)** — 콘솔에 "상품 속성 비교" 목록→상세
화면을 신규 추가했다. 상세 화면은 필드별로 매입처/판매채널/HOMEZ
현재 값을 출처·확인시각과 함께 나란히 표시하고, 불일치·확인불가
항목마다 라디오(발견된 값들)+직접입력 중 하나를 사람이 고르게
한다(서버가 기본값을 미리 선택하지 않음). 기존 HOMEZ 콘솔 패턴
(`responsive-cards` 테이블, `.pill` 상태뱃지, 목록→상세 네비게이션)을
그대로 재사용했다. **이 세션에서 실제 브라우저로 렌더링·클릭
동작을 검증하지 못했다**(Node 문법 검사만 통과) — 실제 DB에 접근하는
로그인이 필요한 검증이라 이번 라운드에서 실행하지 않았다.

**Phase 9I/9J(10-17/10-18, 리콜·판매중지 확인 및 차단)** — 신규
도메인 `app/domains/recall_notice`. 공식 데이터 소스가 아직
선정되지 않아(공통 규칙 15) `RecallNoticeProvider` Protocol 계약과
`FakeRecallNoticeProvider`(부분 실패 시뮬레이션 지원)만 구현하고
실제 네트워크 Provider는 만들지 않았다 — `get_real_provider()`는
의도적으로 항상 `NotImplementedError`를 던진다. 매일 확인
스케줄러 Job을 등록했지만 자동화 모드 기본값은 PAUSED다(사용자가
명시적으로 켜기 전까지 아무 일도 하지 않는다). 차단
(`RecallProductBlock`)은 회사별로 완전히 격리되고, 실제 발주와
실제 등록 양쪽에 배선했다. 기존 주문은 자동취소하지 않고 사용자
확인대상으로만 분리한다. **"서버 관리자" 전용 알림 채널이 이
저장소에 아직 없어**, 회사 SUPER_ADMIN 알림은 실제로 발송하되
"서버 관리자" 몫은 감사로그 기록으로만 대체했다 — 이 부분과
"가격 확대"/"가상재고 증가" 전용 call-site 미배선이 10-18을
PARTIALLY_IMPLEMENTED로 남긴 이유다.

**Phase 9K(8-16 재감사)** — 이미 구현된 8-16의 연결별 독립 카운터,
회사간 교차알림 없음, 재시작 후 카운터 지속을 각각 전용 회귀
테스트로 추가 검증했다(기존엔 구현만 있고 이 세 속성을 직접
증명하는 테스트가 없었다). 문제는 발견되지 않았다 — 설계 자체는
이미 올바르게 격리돼 있었고, 이번 라운드는 그 사실을 회귀로
고정했을 뿐이다.

**결함 수정(연쇄 회귀)**: Phase 9G/9F의 새 컬럼·새 테이블이
기존 lightweight 테스트 fixture(경량 `Base.metadata.create_all
(tables=[...])` 패턴을 쓰는 파일들)에서 반복적으로 "no such table"
오류를 일으켰다 — `test_purchase_channel_connection_service.py`,
`test_purchase_order_submission_service.py`, `test_listing_wizard_
service.py`(및 이를 임포트하는 `test_coupang_live_submission.py`)
전부에서 이 세션 자신이 만든 새 테이블을 fixture에 추가해 수정했다
(Phase 3/9/10A에서 반복된 것과 동일한 클래스의 유지보수 결함 —
매번 직접 재현·수정했다). 별도로 `test_purchase_channel_connection_
migration.py`의 컬럼-스트립 정규식이 `VARCHAR(500)`처럼 괄호 있는
타입을 만나면 잔여 문자열을 남기는 버그를 발견·수정했다(9A~9C가
추가한 `order_paused_reason` 컬럼 때문에 처음 드러남 — Phase 9의
`NOT NULL` 처리 버그와 같은 계열).

**신규 알림 이벤트**: `SUPPLIER_CONNECTION_ORDER_AUTO_PAUSED`,
`VIRTUAL_STOCK_ZERO_PROPOSED`, `RECALL_PRODUCT_BLOCKED` 3개를
`wired=True`로 등록하고, `tests/test_notification_delivery_pt3.py`의
하드코딩 목록(신규 이벤트를 추가할 때마다 갱신해야 하는, 이 파일에서
이미 여러 번 겪은 유지보수 패턴) 2곳을 매번 함께 갱신했다.

**신규 Migration 5개**(`20260915_05`~`20260915_09`) — 전부 임시
SQLite에서만 검증, 실제 homez.db는 건드리지 않았다.

### 13.12 Phase 10B — 안전 기능 전체 구현 후 최종 전체 회귀

HEAD=`f37a092`(작업 시작 시점 origin/main과 일치) 기준으로
`python -m unittest discover -s tests -p "test_*.py" -v`를
1회 실행했다(실제 homez.db·실제 Windows Credential Manager·실제
외부 API 전혀 미사용).

**결과**: 4,411건 실행, 6,383.397초, 실패 2건, 오류 0건, skip 7건
(Phase 6이 게이트한 실제 Windows Credential Manager 테스트 —
의도된 skip, 13.6 참고).

**실패 1 — 원인과 수정**: `test_permissions_timestamps_migration.py
::test_migration_directory_contains_only_expected_files` — 이번
세션이 추가한 신규 Migration 5개(`20260915_05`~`09`)가 이 파일의
하드코딩된 "예상 Migration 파일 목록"(`SUBSEQUENT_MIGRATIONS`)에
없었다. 이 파일 자체의 docstring이 이미 "정당한 신규 Migration
추가 시 이 목록도 갱신해야 한다"는 유지보수 규칙을 여러 차례
기록해 온, 이 세션(과 이전 세션들)에서 반복적으로 재발한 것과
동일한 클래스의 결함이다. 5개 파일을 각각의 추가 사유 주석과 함께
목록에 추가해 수정했다. 집중 재검증: 같은 파일 13/13 통과.

**실패 2 — 원인과 수정**: `test_payment_domain.py::
test_register_method_never_stores_raw_card_number_in_db` — 이
세션이 만든 코드와 무관한, **기존에 잠재해 있던 flaky 테스트**였다.
`payment_methods.credential_target_name`은 `FakePaymentProvider.
tokenize()`가 `raw_details`를 전혀 보지 않고 `secrets.token_hex()`
만으로 생성한 토큰의 접미사(무작위 16진수)인데, 테스트가 3자리
CVC("999")를 그 컬럼 전체 텍스트에서 부분일치로 찾고 있었다 —
무작위 16진수 문자열이 우연히 "999"를 부분 포함할 확률이 무시할
수 없는 수준이라(약 0.3%/회), 실제 유출이 전혀 없어도 이따금
실패할 수 있는 구조였다(이번 회귀에서 실제로 그 우연이 발생함 —
`43d9999a67236b75`). 16자리 카드번호 검사는 우연 충돌 확률이
무시할 수준이라 그대로 뒀고, CVC는 사용자가 직접 입력하는 자유텍스트
필드(`display_name`)에서만 확인하도록 검사 범위를 정확히 좁혔다
(검증 의도 자체는 그대로 유지 — 완화가 아니라 잘못된 비교 대상을
바로잡은 것). 집중 재검증: 같은 파일 30/30 통과(신규 assertion을
5회 반복 실행해 재발하지 않음도 확인).

**이 두 실패 모두 완화·스킵·예외처리로 숨기지 않았다** — 첫 번째는
목록을 실제 상태에 맞게 갱신한 것이고, 두 번째는 검사 로직 자체의
결함(무작위 데이터를 상대로 한 부적절한 부분일치 검사)을 근본
수정한 것이며, 카드번호·CVC가 DB에 저장되지 않는다는 실제 보안
불변식은 조금도 약화되지 않았다.

**Migration 목록 갱신 파일 수 정정**: `docs/HOMEZ_PROJECT_STATE.md`
등 다른 문서가 "이 세션에서 3개 테스트 파일을 갱신했다"고 기록한
이전 라운드와 달리, 이번 라운드는 이 목록 관리 패턴을 쓰는 파일
중 실제로 실패를 일으킨 것이 `test_permissions_timestamps_
migration.py` 1개뿐이었다(다른 하드코딩 목록 파일들은 이번에 추가한
Migration 대상 테이블을 별도로 나열하지 않는 구조라 영향받지
않았다).

### 13.13 Phase 11 — 223개 세부기준 재판정

`docs/HOMEZ_USER_OPERATION_SETTINGS_AUDIT_20260909.md`의 개별 표
223개 행 중, 이번 세션(Phase 9A~9K)이 실제로 갱신한 11개 행
(7-8/7-11/7-16/8-5/8-6/8-16/8-19/10-4/10-5/10-17/10-18)을 최신
코드·테스트 근거로 다시 썼다. **이전 회귀(Phase 10A, 13.10)의
문서 프레이밍이 "10개 항목이 모두 끝난 것처럼" 읽힐 수 있었던
부분을 여기서 바로잡는다** — 실제로는 7개(7-8/7-11/8-5/8-6/8-16
/8-19/10-4)만 IMPLEMENTED이고, 4개(7-16/10-5/10-17/10-18)는 각각
서로 다른 이유(독립 게이트 미승격/브라우저 미검증/외부 데이터
소스 미선정/서버 관리자 채널 부재)로 PARTIALLY_IMPLEMENTED다.

그 문서의 전체 223개 세부기준 요약표는 2026-09-09 Phase 0
정합화 이후 한 번도 재계산되지 않은 채 옛 기준선(58/52/84/29)
그대로 방치돼 있었다 — 그 사이 여러 세션이 개별 행을 수십 건
갱신했는데도 요약표는 그대로였다. 이번 Phase 11에서 **223개 행
전부를 프로그램으로 다시 파싱**(정규식, 수기 집계 아님)해
재계산했다: **구현완료 83, 부분구현 58, 미구현 52, 검증필요 30,
합계 223**(검산 통과). 이 변화(58→83 등) 중 이번 세션이 직접
만든 몫은 위 11개 행뿐이고, 나머지는 2026-09-09~09-14 사이의
이전 세션들이 이미 만들어 둔 것을 처음으로 정확히 집계에 반영한
것이다 — "이번 세션이 그만큼을 새로 구현했다"는 뜻이 아니다.

**"회귀 통과"와 "실제 운영 검증 완료"의 구분**: 이번 세션이
IMPLEMENTED로 판정한 7개 항목은 전부 임시 SQLite+Fake
Provider+InMemoryCredentialStore 기반 자동화 테스트를 통과했다는
뜻이지, 실제 온채널 계정·실제 쿠팡 API·실제 homez.db로 단 한 번도
실행해 본 적이 없다는 뜻이다(이 세션 전체의 공통 제약 — 공통 규칙
5·6과 동일). 10-5(비교 UI)는 코드 작성은 끝났지만 브라우저
...
- ~~Phase 9 안전 기능 11개 중 8-16만 구현 완료. 나머지 10개 미착수~~
  **[2026-09-15 Phase 9A~9K에서 전부 착수·완료]** 13.11 참고 — 8개
  IMPLEMENTED(7-8/7-11/8-5/8-6/8-16/8-19/10-4 + 기존 8-16), 3개
  PARTIALLY_IMPLEMENTED(7-16/10-5/10-18), 10-17은 외부 데이터 소스
  미선정으로 여전히 PARTIALLY_IMPLEMENTED다(정확히 예상됐던 그대로).
  남은 잔여 위험: (1) 매입처 어댑터 계약이 제조사·원산지·사이즈·
  수량 필드를 갖고 있지 않아 10-4의 5개 필드는 실데이터 자동
  연동이 안 됨, (2) 10-5(비교 UI)는 브라우저 미검증, (3) 10-17은
  공식 리콜 데이터 출처 선정이 외부 답변 대기, (4) 10-18의 "서버
  관리자" 알림 채널과 "가격 확대"/"가상재고 증가" call-site가
  아직 미배선.
