...
**HOMEZ V7 — 기존 상품 후보 이어받아 DB 적용·실거래 준비 (2026-09-23, 10차). 시험상품 여전히 미확정(추천은 ① CH1147184 유지), 원본 Migration 여전히 미승인, 판매신청 게이트는 코드 기준 "미충족"으로 확인.**
...
**신규 발견**: `purchase_sales_application_attempts`에 CH1147184 행이 0건 — 2026-09-14의 실제 판매신청 HTTP 200 접수는 사실이지만 공식 추적 테이블을 거치지 않아, 현재 코드의 `is_sales_application_confirmed()` 기준으로는 **"미충족"**이다. 재신청 시 온채널이 중복 요청을 어떻게 처리하는지는 미확인 — 재신청 여부는 별도 사용자 결정 사항으로 남겼다.
`HOMEZ_V7_DB_DECISION_REVIEW_20260923.md` §6 Migration 승인안에 대한 사용자의 명시적 승인은 이번에도 확인되지 않아 **원본 적용은 계속 보류**한다(대상·해시는 코드 무변경으로 그대로 유효).
...
6차 결정 이후 설치본에 새 업무 데이터가 쌓이지 않았음을 콘텐츠 타임스탬프(최대 2026-08-29, mtime 아님)로 재확인해 결정을 재사용할 근거를 보강했다.
8차의 "두 경로 SHA-256 완전 일치"는 "설치본의 평문 경로 vs Claude 패키지 가상화 섀도 경로" 비교였음을 명확히 정정(두 DB 간 비교가 아님, 절대경로·크기·mtime·해시 표로 재기록).
승인 생성→공급처 옵션 연결 변경→기존 승인 실행 시도 흐름을 신규 격리 테스트 3건(`tests/test_purchase_order_approval_survives_option_link_change.py`)으로 재현: 스냅샷과 다른 옵션(금액은 동일)으로는 실제 발주 함수가 스파이 기준 0회 호출되며 차단, 승인 스냅샷 자체는 DB 재조회로 불변 확인, 재승인 후에는 정상 실행(1회 호출) 확인 — 가드 차단 0건.
모바일 시각 검증을 `scale=1` 스크린샷으로 재시도해 성공(8차의 "DOM 측정만" 한계 해소) — 겹침·잘림·접근 불가 버튼 0건.
8차의 "실제 DB 재조회" 표현을 "격리 DB(`adopted_e2e.db`) 저장·재조회"로 정정(합성 DB였음을 명확히).
제품 코드 결함은 발견하지 못했다.

**원본 적용 승인안(§6, 저장소 루트 DB 대상, 이 문서 작성 직전 재계산)**: 대기 Migration 2개(`20260918_00_...`, `20260921_00_...`, 해시 포함) — 8차 §5-1 사본 리허설 결과(코드·Migration 무변경이라 재사용) 그대로 유효. 설치본은 이번 승인안에 포함하지 않는다(기존 결정대로 별도 대상).

**남은 승인(전부 독립, 미실행)**: ①저장소 루트 DB Migration 실제 적용(백업+적용 직전 재확인 후, §6 승인안) ②쿠팡 상세조회(대상 미확정, 총 2회 한도) ③온채널 조회(대상 미확정, 총 4회 한도) ④시험상품·실제 등록·판매신청·주문·결제 ⑤설치본 DB 대상 별도 승인안(필요 시 추후 별도 제출).
**"원본 적용 승인안 확정 제출"까지 완료했다. "원본 적용 완료"·"V7 실사용 완료"는 아니다.**

---

**HOMEZ V7 — 옵션 연결 채택 이후 업무 DB 적용 준비: 사본 조사·리허설·화면 검증 완료, 원본 미적용 (2026-09-23, 8차). [9차에서 정정: "대상 DB 정체성 미확정"은 오류 — 기존 6차 결정(저장소 루트)이 이미 있었다. 상세: 위 9차 항목.]**

**결과(상세: `docs/HOMEZ_V7_DB_UPGRADE_PREP_20260923.md`)**: 실제 HEAD가 `f776a3b`(=원격, 7차 문서 커밋)임을 재확인, 이후 코드 변경이 없어 `ec44aae` 전체 회귀(4,677/0/0/25, 가드 차단 0)를 재사용했다(재실행 안 함).
실제 DB 후보 2곳을 읽기 전용으로 조사: 저장소 루트(`homez.db`, 대기 Migration 2개)와 설치본(`%LOCALAPPDATA%\HOMEZ\data\homez.db`, 대기 34개, 2026-08-24 사고 당시 "실제 운영 DB"로 지칭된 경로) —
**어느 쪽이 유일한 공식 업무 DB인지 이번에도 단정하지 않았다**(상충하는 신호를 그대로 기록, 승인안에서 사용자 결정 요청). 두 후보 모두 `sqlite3.Connection.backup()`으로 저장소 밖(`Homez-Backups\db_investigation_20260923\`)에 사본을 뜨고 원본 SHA-256 불변을 재확인했다.
사본에서 각각 2개/34개 Migration을 실제로 적용·롤백 리허설(무결성·FK·컬럼 단위 데이터 보존·ORM·옵션 연결 저장/재조회 전부 확인, 재적용 자동 차단 확인). 설치본에서 발견된 3개 테이블의 컬럼 추가는 전부 의도된 additive 변경(기존 값 변경 0건)임을 컬럼 단위로 확인했다.
합성 DB+Fake Provider+격리 Credential Store로 옵션 연결 5개 시나리오(정상/SKU없음/식별자충돌/수량환산/옵션누락)를 실제 채택 코드 화면에서 저장→하드리로드 후 API 재조회로 검증, 모바일(375px)도 DOM 측정으로 확인. 제품 코드 결함은 발견하지 못했다.

**남은 승인(전부 독립, 미실행)**: ①대상 DB 확정(사용자 결정, §8-1) ②그 DB에 Migration 실제 적용(백업+직전 재확인 후) ③실제 DB 사본 접근(리허설용, 이미 이번에 완료) ④쿠팡 상세조회(대상 미확정, 총 2회 한도) ⑤온채널 조회(대상 미확정, 총 4회 한도) ⑥시험상품·실제 등록·판매신청·주문·결제.
**"사본 업그레이드 검증 완료"는 달성했다. "원본 적용 완료"·"V7 실사용 완료"는 아니다.**

---

**HOMEZ V7 — 옵션 연결 v2 채택 완료(실제 저장소, 실제 DB 미적용) (2026-09-23, 7차). 실제 실행(Migration 적용·조회·등록·주문·결제)은 전부 승인 대기.**

**결과(상세: `docs/HOMEZ_V7_OPTION_LINK_V2_ADOPTED_20260923.md`)**: 6차까지 스크래치에서 검증한 옵션 연결 v2 패치(25개 파일, 신규 Migration 1개)를 **실제 저장소 코드로 채택**했다(커밋 `8fcf54d`).
판매 옵션↔공급처 옵션 연결 저장·재사용, 식별자 결정표(SKU/vendorItemId 경로 분리·충돌 차단·이름·순서 매칭 금지), 등록 결과 옵션번호 부착, 서버 확정 준비 판정이 이제 저장소 코드에 있다.
채택 검증 중 옵션 연결과 **무관한** 결함 1건을 새로 발견해 수정했다(`ec44aae`): Migration 승인 성공 후 재계산 경로(`refresh_restricted_mode_state`)가 격리되지 않아 실제 `homez.db`를 읽기 전용으로 열려던 것 — 가드가
차단, 테스트 자체는 통과라 결과만 보고는 드러나지 않았다. 신규 Migration은 다른 74개와 같은 LF 바이트(SHA-256 `1a8c65c95d24…821a`)로 저장소에 있으나 **실제 DB에는 적용하지 않았다**(적용 시 제한 모드).

**검증**: 패치 105건 + 인접 432건을 이 저장소에서 재실행해 전부 통과, 최종 고정 기준선(`ec44aae`) 전체 회귀 **4,677건 / 실패 0 / 오류 0 / skip 25(6차와 이름까지 동일) / 가드 차단 0건 / 종료 코드 0**,
실제 `homez.db` 전후 완전 동일. 화면→API→Service→저장소→Provider 연결과 필수 실패 조건(식별자 충돌·이름 매칭 금지·회사 혼입·승인 후 불변·부분 성공/재시작)을 코드 추적 + 기존 테스트로 확인했다.
서버는 기동하지 않았다(제한 모드 위험 회피, import 스모크만 확인). 브라우저 E2E는 내용이 스크래치 검증본과 100% 동일해 재실행하지 않고 이전 결과를 재사용했다.

**커밋(전부 push 완료)**: `8fcf54d`(옵션 연결 v2 채택) → `ec44aae`(무관 결함 수정) → 이 문서.
**남은 승인(전부 독립, 미실행)**: Migration 실제 적용(20260918→20260921, 백업+사본 리허설 후) / 실제 DB 사본 접근 / 쿠팡 상세조회(대상 미확정, 총 2회 한도) / 온채널 조회(대상 미확정, 총 4회 한도) /
`.gitattributes`(이번 범위 제외) / 시험상품·실제 등록·판매신청·주문·결제. **V7 실사용 완료가 아니다 — 실제 거래 전에는 선언하지 않는다.**
...
**커밋(전부 push 완료)**: `9656b9d`(격리 게이트+가드 도입) → `6203b2d`(가드 교착 제거+데스크톱 자격증명 격리) → `317bc39`(발주 재시작 자격증명 격리) → `5ee8127`(데스크톱 run() 자격증명 격리) → `692d1a5`(문서)
→ `e874c5f`(후속: 실제 접근은 없었으나 같은 패턴이던 잠재 위험 2개 파일 방어적 격리, 11건 통과·차단 0건).
...
내용 차이 0건이다. **정정 1**: 신규 Migration의 실제 채택 바이트(LF) SHA-256은 **`1a8c65c95d24…821a`**(이전 문서의 `aebbd3fd…9884`는 CRLF 값). **정정 2**: 이전 전체 회귀들은 실제 DB 접근을 차단하지 않았고, 일부 기존
테스트가 경로가 고정된 실제 DB 2곳(저장소 `homez.db`, `%LOCALAPPDATA%\HOMEZ\data\homez.db`)을 **읽기 전용**으로 열었다 — 쓰기 없음, 두 파일 크기·수정시각 불변, 그러나 "실제 DB 미접촉" 서술은 회귀 부분에서 사실이 아니었다.

**실패 2건·skip 16건 확정**: 실패 = 런처 테스트(venv 없음) + 실제 DB 대조 테스트(중간에 생긴 0바이트 `homez.db`). 0바이트 생성 원인은 `test_migration_restricted_mode`의 미들웨어 테스트가 알림 경로로 기본 DB에
접속하던 **테스트 구성 결함** — 테스트 2개만 수정(제품 코드 무변경, 단언 추가, 변이 검증 통과). skip = 의도된 7(Credential Manager 옵트인) + 실제 DB 없음 9(수정 후 11).

**최종 고정 트리(HEAD+패치+테스트수정, LF) 전체 회귀 — 2차(감사 훅으로 실제 DB·백업 open 전부 차단)**: 4,656 실행 / 성공 4,632 / 실패 0 / 오류 6 / skip 18 / 종료 코드 1 — **전체 통과가 아니다.** 오류 6은 모두 실제 DB를
열려던 테스트가 가드에 막힌 것(메서드 5 + import 시점에 실제 DB를 해시하는 모듈 1). 실제 DB 의존 29건(오류 5·미실행 13·skip 11)은 검증되지 않았다. 옵션 연결 관련 105건과 체크섬 잠금 테스트는 통과.

**적용 계획**(같은 문서 §6): 두 파일이 대기 중일 때 하나만 승인하면 아무것도 적용되지 않음을 리허설로 확인 → 권장 순서 = 백업 → 사본 리허설 → 20260918 단독 적용 → 코드 채택 → 20260921 단독 적용. 롤백 3종(코드 되돌리기 / 백업 복원 /
수동 DROP) 확인, 구 코드 + 신 DB는 대기·오류 없음. **실제 조회 계획**(§7): 쿠팡 상세 조회 총 2회·온채널 총 4회 한도(대상 미확정, 등록·시험상품 미선정), 호출 횟수를 코드가 강제하지 않음.

**승인 대기(각각 독립)**: 코드 채택 / 20260918 적용 / 20260921 적용(체크섬 `1a8c65c9…`) / 실제 DB 접근(백업 복사·사본 리허설·읽기 전용 진단) / 실제 저장소 읽기 전용 회귀 / 쿠팡·온채널 조회 / `.gitattributes` /
시험상품·실거래. **대상 DB 확정 필요**(개발 모드 저장소 DB vs 설치본 DB). **결정 필요**: 실제 DB 읽기 테스트를 기본 회귀에서 제외할지. 이번에 저장소에 들어간 것은 테스트 2개 수정과 문서뿐이다. V7 완료·실운영 준비 완료가 아니다.

---

**HOMEZ V7 — 옵션 연결 채택 준비·화면·식별자 연결 완성 (2026-09-21, 4차). 코드·Model·Migration 채택 승인 대기, 실제 DB 미적용.**

**결과(상세: `docs/HOMEZ_V7_OPTION_LINK_ADOPTION_20260921.md`)**: 첫 확인한 판매 옵션↔공급처 옵션 연결을 저장하고 다음 주문부터 재사용하는 체인을
스크래치 복제본(동결 `dc97d1f`)에서 완성했다. 산출물은 `docs/proposals/20260921_supplier_option_link.patch`(v2, 25개 파일)뿐이며
**저장소 코드·`migrations/`·실제 `homez.db`에는 반영하지 않았다.** 식별자는 SKU 경로(등록 `externalVendorSku` = 주문 `externalVendorSkuCode`)와
`vendorItemId` 경로를 서로 다른 값으로 취급해 결정표로 해석하고(충돌은 차단, 이름·배열 순서 매칭 없음), 등록 결과 연결은 쿠팡 상품 상세 조회
계약(공식 문서)으로만 옵션번호를 부착하며 미발급·부분·충돌·결과불명은 "준비 미완료"로 남긴다. 준비 완료는 서버가 확정한 전체 옵션 목록 기준이고
클라이언트 SKU 일부만으로는 "선택 범위 확인"까지만 표시한다. 콘솔에 매입 검토·상품 준비 결과 화면을 추가했다(조회 결과에서 옵션 선택, 저장값 미리 채움,
명시적 변경·해제, 승인 스냅샷 불변).

**검증**: 옵션 연결 신규/재작성 테스트 105건, 변이 검증 10건 전부 검출, 브라우저 E2E(임시 DB·가짜 쿠팡/온채널·소켓 가드, 외부 접속 0건)에서 결함 4건을
찾아 수정, Migration 리허설(빈 DB 신규 설치 75개 / 기존 데이터 업그레이드 / 최신 코드+미적용 DB) 통과. **동결본 전체 회귀: 4,668 실행 / 성공 4,650 /
실패 2 / 오류 0 / skip 16 — 전체 통과가 아니다.** 실패 2건은 복제본에 venv·실제 DB가 없는 환경 문제이며 패치 없는 기준선에서도 동일 조건에서 같은 실패·skip이
재현됨(문서 §10). 실제 DB를 읽는 테스트는 패치 적용 상태의 실제 저장소에서 아직 실행하지 못했다.

**미검증(실제)**: 쿠팡 상세 조회 응답의 실제 형태(문서 계약 기준 구현), 온채널 실제 동작, 실제 DB 적용·제한 모드 거동, 실제 DB 사본 업그레이드(접근 승인 미확인 →
미접근), 실제 등록·판매신청·주문·발주·결제. **승인 대기**: ① 코드 채택 ② 실제 DB Migration 적용(백업·사본 리허설 후) ③ 쿠팡 상세 조회 읽기 전용 ④ 실제 DB 사본 접근 ⑤
시험상품·실거래 관련 ⑥ 줄바꿈 고정 별도 작업. V7 완료·실운영 검증 완료·실제 주문 실행 준비 완료가 아니다.

---

**HOMEZ V7 — 상품 옵션 등록·공급처 옵션 연결 조사·설계·격리 검증 (2026-09-21, 3차). Model·Migration 채택 승인 대기.**

**조사 결과(설계 문서 `docs/HOMEZ_V7_OPTION_LINK_DESIGN_20260921.md`)**: A(옵션 등록)는 입력·저장·쿠팡 전송·결과불명
보존까지 구현돼 있으나 ① SKU 검증 결함(빈 값·공백 포함이 통과) — **수정·커밋(`71f2b0e`)**, ② 옵션별 쿠팡
식별자(`vendorItemId`) 저장 미구현, ③ 등록 결과 조회 응답 형식 검증필요(추측 안 함). B(옵션 연결)는 쿠팡 주문
→ HOMEZ SKU(`OrderSkuResolution`)까지만 있고 **HOMEZ/쿠팡 SKU → 온채널 상품코드·옵션ID 영구 저장·재사용은
미구현**(발주 검토는 운영자 입력, `schema.py:509`). 이전 D3 준비서의 "새 기능이라 범위 밖" 판단은 사용자
지시로 철회했다.

**설계·격리 검증(스크래치, 운영 코드 미반영)**: 조인 키 = 쿠팡 `externalVendorSku`(= 주문 `channel_sku`), 신규
테이블 `supplier_option_links` 1개(UNIQUE(회사, 스토어 연결, SKU), `units_per_sale`, ACTIVE/NEEDS_REVIEW/
DISABLED, 버전 CAS). 저장 전 그 매입 계정으로 공급 상품 실조회·옵션ID 소속 확인, 공급 옵션 삭제·이름 변경·
계정 변경 시 조용한 대체 없이 차단, 승인 시 서버 재대조, 자동 매입 준비는 전 SKU ACTIVE일 때만. 격리 테스트
39건 3회 반복 OK, 변이 검증 4건 검출, 집중 회귀 663건 중 2건 실패(패치 무관 기존 환경 특성 — 패치 없는 새
복제본에서도 동일, 설계 문서 §8). 전체 회귀는 실행하지 않았다(재실행 금지 지시, 채택 후 1회 예정).

**Migration**: 신규 `20260921_00_create_supplier_option_link_schema.sql`(SHA-256 `e8214dc6…611b59`)은
`docs/proposals/20260921_supplier_option_link.patch`에만 있고 **저장소 `migrations/`에도 실제 DB에도 미반영**
(기존 미적용 1개 `20260918_00_...` 그대로). 임시 DB 복사본 검증 리허설 통과(무결성 ok·기존 테이블 무변경·재적용
차단·롤백 복원). 화면(콘솔)과 등록 결과 식별자 배선은 미구현. 실제 쿠팡 등록·판매신청·주문·발주·결제 없음.
시험상품은 여전히 미확정, 쿠팡 시험 주문 정책 미확인. V7 완료·실운영 검증 완료·실제 주문 실행 준비 완료가 아니다.

---

**HOMEZ V7 — 동시성 실패 원인 규명·수정 + 상품 조회 근거 복원 + D3 준비 (2026-09-21).**

**동시성 실패(`test_concurrent_verify_info_only_one_succeeds`) — 테스트 하네스 결함, 제품 코드 무변경**:
실패 실행의 실제 traceback에서 `InterfaceError`/`ObjectDeletedError`가 모두 워커의 인자 `candidate.id`
평가 중(메인 스레드 Session의 만료된 객체를 두 스레드가 동시에 새로고침)에 났고 서비스 코드에는
닿지 않았음을 확인했다. 제품의 원자적 조건부 UPDATE는 정상이며, 그 조건을 제거한 사본에서는 새
테스트가 3/3 결정적으로 실패한다(mutation). 수정: id를 원시 값으로 고정, 두 요청이 사전 검사를
통과한 뒤 쓰기 직전에 barrier로 충돌 시점을 고정, 정확히 1 ok + 1 ConflictException·이벤트/디스패치
1회·근거 행 종류별 확인. 상세: 감사 문서 34절.

**고정 기준선 전체 회귀(HEAD `a32623e`, 수정 후 1회)**: **실행 4,563건 / 성공 4,556 / 실패 0 / 오류 0 /
skip 7 / exit 0.** 이전 회귀(3703273 기준 실행 4,563 / 성공 4,555 / 실패 1 / skip 7 / exit 1)의 기록은
그대로 두며 두 결과를 합산하지 않는다("4,562건 통과" 표현은 skip 포함 오류로 철회). 외부 접속 차단
0건, `homez.db` SHA-256·크기·mtime 전후 동일(관측 사실). 상세: 감사 문서 35절.

**Migration**: 미적용 1개(`20260918_00_...`) 그대로, 원본 미적용.
**시험상품·정책·다음 실행**: 위 2026-09-20 기록 그대로 — 시험상품 미확정, 쿠팡 시험 주문 정책 미확인(문의
초안 미발송), 실제 주문 보류. CH1147184의 과거 API 관측(옵션 4개 등)을 복원했고 등록 초안·매핑표
초안(D3 준비서 2-6절)을 준비했다(등록·DB 쓰기 없음). "D3 준비 중"이며 실제 주문 실행 준비 완료도 V7
완료·실운영 검증 완료도 아니다.

---

**HOMEZ V7 — 사용자 상품 3종 비교 + 한도 수정 최종 검증 + D3 준비 (2026-09-20, 2차).**

**기준선·최종 회귀(2026-09-20 기록, 2026-09-21 정정)**: HEAD `3703273`(원격 일치). 고정 기준선
전체 회귀 1회(외부 접속 차단 가드 적용): **4,563건 실행 / 실패 1 / 오류 0 / skip 7 / exit 1
— 전체 통과가 아니다.** unittest 집계로 성공은 **4,555건**이다(이전에 적은 "4,562건 통과"는
skip을 성공에 섞은 표현이라 철회). 실패 1건은 상품후보 도메인의 2스레드 동시성 테스트
(`test_concurrent_verify_info_only_one_succeeds`)로 `purchase_task`와 무관하다. "이전 기준선에서도
났다"는 관측을 안전성의 근거로 쓰지 않으며 고정 확률로도 단정하지 않는다 — 원인은 2026-09-21에
실제 traceback으로 규명했다(감사 문서 34절). 회귀 중 외부 접속 차단 시도 0건, `homez.db`
SHA-256·크기·mtime 전후 동일(관측 사실). 이전 집중 회귀(443건+32건)와 합산하지 않는다.
미호출 예약 만료 함수의 향후 보호 조건은 감사 문서 33절(현재 실행 결함 아님).

**사용자 상품 3종 비교(D3 준비서 1-1·2-1절, 2026-09-21 정정)**: ① CH1147184는 이전 Codex의
실제 API 상세조회에서 **옵션 4개**(3945578 샴푸 200ml 본품 7,150원 / 3945579 샴푸 500ml 리필
11,000원 / 3945580 바디워시 200ml 본품 5,050원 / 3945581 바디워시 500ml 리필 7,850원)·기본 배송비
3,000원·옵션별 `qty` null이 관측됐다(시점 미상, 최신 아님 — "추정"이 아니라 과거 API 관측).
② CH5250918은 배송기간 "1일~2121일"·KC "[전기용품]안전인증" 표기를 확인 필요 사항으로 유지.
③ CH4922462는 수량별 배송비(1개마다 부과)·약 10kg 추정 무게를 비용 검토에 반영하되 무게만으로
반품 가능 여부를 단정하지 않고, "상세정보 별도표기"는 상세 이미지 확인 전 정보 부재로 확정하지
않는다. 입점일자로 공급 신뢰도를 판정하지 않는다. 공급가·재고는 로그인/API 필요로 ②③은 미확인,
①도 현재값 미확인. 새로 조사한 세탁망·책꽂이·거름망은 참고용 추가 후보로만 구분. 시험상품은
확정하지 않았다(현재 근거로 ①을 우선 검토 대상으로 추천, 최종 선택은 사용자). 지역 배송비는
API 필드와 공개 화면 "추가" 표기의 합산 의미를 확인하기 전 최종금액으로 확정하지 않는다.

**시험 구매 정책**: 쿠팡 공식 문서(금지 행위 가이드라인, 판매자 관리 정책, FAQ)에서 시험 주문
방법·본인 구매 허용 조건을 찾지 못했다 — "금지 문구 없음"을 "허용"으로 해석하지 않고, 사용자
위험 수용으로 대신하지 않는다. 판매자센터 문의 초안 작성(WING 로그인 권한 없어 미발송, 발송
승인 필요). 확인 전 실제 시험 주문 보류, 상품정보·등록 초안·매핑 준비는 계속.

**Migration**: 미적용 1개(`20260918_00_...`, SHA-256 `93ca0724...b4b1`) 그대로, 원본 미적용.
**API 조회 계획**: 선택 상품 상세 `GET /openapi/seller/product/{코드}` 연결 4, 상품당 1회·최대 3회,
재시도 없음(미실행, 승인 대상). 온채널 판매신청은 별도 시험 항목이 아니라 공식 계약상 발주
전 준비 절차(2-5절, 승인 없이 신청·생략하지 않음).

"D3 준비 중"이며 "실제 주문 실행 준비 완료"도 V7 완료·실운영 검증 완료도 아니다.

---

**HOMEZ V7 — 검증 마무리 + 한도 보호 실제 공백 수정 + 시험상품 후보 조사
(2026-09-20).**

**기준선**: 원격·로컬 HEAD `3c7716b`, 전체 회귀 4,560건 / 실패 0 / 오류 0 / skip 7
(3c7716b 기준, 회귀 시작 시각보다 이후에 수정된 소스·테스트 파일 없음을 mtime으로
확인). 이후 이번 라운드 코드 변경분은 아래 집중 회귀 범위만 검증했으며, 4,560건
결과를 이 변경분까지 확대하지 않는다.

**집중 회귀(purchase_task 도메인 17개 스위트, 464건)**: 첫 실행에서 오류 21건이
전부 `tests/test_purchase_order_approval_service.py` 한 파일에서 났다 — 새 집계가
`purchase_order_submission_attempts` 테이블을 참조하는데 이 파일의 임시 DB
픽스처에만 그 테이블이 없었기 때문(운영 스키마에는 항상 존재, 코드 결함 아님).
픽스처에 테이블을 추가하고 해당 파일 32건 재실행 통과. 나머지 443건은 첫 실행에서
통과했고 그 뒤 앱 코드는 바뀌지 않았다(mtime 확인).

**로그 끝 한글 3줄 출처 확정**: `app/desktop/main.py`의 `print()`를 fail-closed
테스트 3건(`tests/test_live_gate4_fix_defects.py`)이 호출한 **테스트 부수 출력**.
해당 파일만 단독 실행해 `OK` 뒤 동일 3줄을 재현했다(stdout 블록 버퍼링 vs
stderr 순서). mtime 불변은 관측 사실로만 기록한다.

**한도 보호 실제 공백 수정(2건, 재현 테스트 3건)**: (1) 발주 승인 단계 일간/월간
한도가 결과 미확정(IN_FLIGHT 잔존·UNKNOWN 미확정) 발주 금액을 승인 만료(10분)
뒤 빠뜨리던 문제 → 만료 후에도 집계에 포함, 사람이 "주문 없음"으로 확정하면
제외. (2) 이미 나간 외부 지출을 운영 가능 금액이 모자라면 기록하지 않던 문제
(이전 라운드 판단 오류) → 초과해서라도 기록하고 가용금액을 음수로 만들어 다음
발주를 차단, 경고 알림. 나머지(RESERVED 미집계 등)는 다른 게이트가 보호함을
코드·테스트로 확인해 결함으로 단정하지 않았다. 상세: 감사 문서 32절.

**시험상품 후보(공개 화면·로그인 없음)**: CH3779623 세탁망(추천), CH2411615
철제 책꽂이, CH5068542 싱크대 거름망 — 가격·옵션·재고는 로그인 필요라 미확인.
정책: 온채널 운영정책(반품 7일·왕복 배송비 판매사 부담), 쿠팡 가이드라인(본인
구매 명시 없음, 판매량·리뷰 조작 금지). 추가 실 API 조회 계획(상세 GET 1회)만
수립, 미실행. Migration 미적용 1개 그대로(원본 미적용). 상세: D3 준비서 2-1~2-4절.

"D3 준비 중"이며 "실제 주문 실행 준비 완료"가 아니다 — 판매신청·쿠팡 등록·매핑·
주문·발주·결제·환불은 실행하지 않았다.

---

**HOMEZ V7 — 지출한도 누락 해소(온채널 트랙) + D3 상품 준비 조사
(2026-09-19).**

**재현·수정한 결함(High)**: 온채널 실 발주 트랙은 `record_purchase()`
(수동 트랙 전용)를 타지 않아 `PurchaseRecord`가 생기지 않고,
일간/월간 지출한도(`PurchaseTaskPolicyService.evaluate()`)의 유일한
근거였던 `sum_recorded_amount_since()`가 `PurchaseRecord`만 읽어
**온채널 실 발주가 아무리 쌓여도 지출한도가 0원으로 보이고 다음
발주 승인이 실제 지출과 무관하게 계속 통과**했다. 실제 판정 결과
(`PurchaseTaskPolicyReason.DAILY_LIMIT_EXCEEDED`)로 재현 후 수정.

**수정 방식(Model·Migration 변경 없음)**: `PurchaseRecord`는 "사람이
직접 입력"을 전제하는 모델이라 자동 실행 코드가 채우지 않기로
유지하고, 대신 양쪽 트랙이 정책평가 시점부터 이미 공유하던
`PurchaseTaskBudgetReservation`을 2단계로 확정한다 — **Stage 1**
(발주 성공 즉시, 송장조회 전, 승인 스냅샷 금액으로 잠정 확정 —
새 상태값 `PENDING_VERIFICATION`)과 **Stage 2**(송장조회로 받은 실 API
확정 금액으로 최종 `CONFIRMED` 승격). 취소·반품이 시작된 뒤에는
지연 도착한 재조회가 예산을 다시 건드리지 못하도록 `task.status`
가드를 추가해 환불이 되돌려지지 않게 했다. 지출한도 집계
(`sum_recorded_amount_since`)는 이 예약들을 `PurchaseRecord`와
이중계산 없이 합산하도록 확장했다. 상세 표: 감사 문서 31절.

**함께 확인(별도 결함 없음)**: 온채널 발주 자체를 막는 연결별
하루/월 발주 한도(`PurchaseOrderApproval` 기반)는 이미 두 성공
경로 모두에서 정상 작동 중임을 코드로 확인 — "테이블이 다르다"는
이유만으로 결함이라 단정하지 않고 실제 동작을 검증했다.

**조회기록 재정정**: "정상판매 1건은 page_size=1 때문"이라던 설명이
실제 호출 로그가 아니라 코드 기본값 추측이었음을 재확인·유지
(2026-09-19 이전 라운드에서 이미 정정, 이번 라운드는 변경 없음).

**시험상품 준비**: 온채널 공개 화면(로그인·API 불필요)에서 후보를
조사하라는 요청은 **실행하지 못했다** — 이 저장소·문서 어디에도
온채널의 실제 공개 웹사이트 URL이 없고, 확인되지 않은 URL을
추측해 접속하지 않는다(안전 원칙). 실제 URL을 알려주면 다음
라운드에서 조사한다.

이번 라운드에서도 실제 업무 DB 변경·자격증명 사용·업무 API 호출·
주문·발주·결제는 실행하지 않았다. 실 Migration 적용도 미실행(pending
1개, 변경 없음).
...
**시험 전용 호출 예산 — 구현 완료.** 쿠팡 주문수집 시험 경로(연결 1개·
ACCEPT·페이지 1장)에 프로세스 재시작·동시 요청·다중 프로세스에도 살아남는
총량(2회) 강제를 추가했다(`order_collection_test_budget_usages` 신규
테이블, Migration `20260918_00`, 실제 별도 OS 프로세스 2개로 원자성 증명).

**반자동 필수흐름 결함 수정(High).** 실제 온채널 발주가 성공해도(`submit_
order()` SUCCEEDED, 또는 RESULT_UNKNOWN을 사람이 ORDER_CONFIRMED로 확정)
`PurchaseTask.status`가 전진하지 않아 송장 등록 화면(TRACKING_REQUIRED
전용, 이미 완성되어 있던 기능)에 도달할 방법이 없었다 — 구 수동(브라우저
구매) 트랙의 `record_purchase()`가 온채널 API 트랙에서 전혀 호출되지
않기 때문이다. 두 성공 경로 모두에서 안전하게(진행 중 상태에서만) 전환
하도록 수정했다.

**정직하게 공개하는 잔여 격차(이번 라운드에서 고치지 않음, 신규 기능
확장에 해당해 범위 밖으로 남김)**:
- 온채널 API 발주의 **실제 청구 금액을 사후에 기록하는 곳이 없다** —
  가장 가까운 값(`PurchaseOrderApproval.item_amount_snapshot`)은 발주
  *직전* 스냅샷일 뿐이다. 다만 이 데이터를 소비하는 정산·마진 기능
  자체가 현재 코드베이스에 없어(`app/domains/settlement/*`는 매출 쪽만
  다룸, grep 0건) 지금 당장 끊기는 기능은 없다.
- HOMEZ가 쿠팡(판매채널)에 송장번호를 자동 업로드하는 기능은 **미구현**
  — 처음부터 운영자가 쿠팡 Wing에 직접 입력하는 수동 경로로 설계됨
  (결함 아님).

**정정 사항**: (1) "회귀 실행 중 코드를 고쳐도 파이썬이 시작 시점 코드만
쓰므로 결과가 유효하다"는 이전 단정을 철회한다 — 지연 import·서브프로세스
스폰 테스트가 섞여 있어 혼합 버전 가능성을 배제할 수 없다(실제 영향
증거는 없으나 단정하지 않는다). (2) "CREATE TABLE만 있어 업그레이드
위험 없음"을 "기존 행 손상 위험은 없음(사본 검증 완료), 원본 적용 자체의
운영 위험(잠금·백업·복구)은 별도"로 정정한다. (3) 온채널 포인트 잔액은
사용자가 5만원 충전 완료를 보고했으나 API로 재확인하지 않았다 — 사용자
보고와 API 확인 사실을 계속 구분한다. 상세 근거는
`docs/audits/20260917_APPROVAL_SCOPE_VIOLATION_AND_ORDER_COLLECTION_STRUCTURE.md`
23~27절.

**전체 회귀**: 고정 기준선에서 재실행 결과는 별도로 기록한다(진행 중).

이번 라운드에서도 실제 업무 DB 변경·자격증명 사용·업무 API 호출·주문·
발주·결제는 실행하지 않았다(온채널 상품 목록/상세 조회 2회는 이 세션
이전에 별도 승인으로 이미 실행된 것이며 이번 라운드에서 추가 호출하지
않았다).

---

**HOMEZ V7 — Phase 7B 진입 전 잔여 4항목 확인·수정 (2026-09-18).**

**항목 2(격리 브라우저 성공 흐름) — 재검증.** 이전 라운드의 UI 검증은 실제
Windows Credential Manager에 없는 참조값을 조회해 400을 받는 "실패 경로"만
확인했을 뿐, 진짜 성공 경로(주문이 실제로 저장되는 흐름)는 검증하지
않았었다. 이번에는 FastAPI 공식 오버라이드 지점으로 `InMemoryCredentialStore`
+ Fake Provider를 실제 서비스 경로에 주입하고, 네트워크·자격증명 tripwire를
추가해(둘 다 발동 안 함) 빈 결과 → 합성 신규 주문 1건(ACCEPT) → 같은 주문
재조회까지 실제 브라우저(데스크톱+모바일)로 확인했다. Order/fulfillment/
PurchaseTask/중복 카운터/체크포인트를 DB와 전부 대조했다 — 정상.

**항목 3(복구 후보 보존) — 계약 보강.** `RecoveryReviewCandidate`에
company_id/store_connection_id/마스킹된 shipment_box_id를 추가하고, 새
테이블 없이 기존 `OrderChannelFulfillment`에서 다시 계산하는 읽기 전용
`list_recovery_review_candidates()`를 추가했다. 응답에만 존재하지 않고
DB에 지속 보존됨, 프로세스 재시작(새 세션)·재조회(중복 없음)·이후 단계의
무관한 실패(후보는 살아남고 체크포인트만 전진 안 함) 전부 유닛 테스트와
격리 브라우저 양쪽으로 확인했다.

**항목 4(무접촉 보고) — 정정.** "`homez.db` mtime 불변 = 무접촉"이라는
이전 서술은 부정확했다(mtime은 쓰기 없음의 증거일 뿐 읽기 포함 무접촉의
증거가 아니다). 재조사 결과 `DATABASE_URL` 미설정 시 기본값이 실제
`homez.db`를 가리키는 것을 확인했으나, `tests/` 전체에 `TestClient(app)`
사용 0건·전역 `SessionLocal` 직접 사용 1건(자체적으로 DATABASE_URL을
스크래치로 덮어씀, 게다가 어떤 test_*.py에서도 참조 안 됨)·실제 서버를
다루는 테스트는 DB 경로를 자체 mock함을 확인해, 구조적으로 무접촉일
가능성이 높다는 더 강한 근거를 찾았다 — 다만 200여 개 파일 전부를 감사한
것은 아니므로 "확인됨"이 아니라 "구조적 근거로 뒷받침되는 높은 확신"까지만
주장한다. 이번 라운드의 격리 브라우저 검증(항목 2)은 이 문제가 없다
(DATABASE_URL 명시 설정 + 의존성 오버라이드 + tripwire 무발동으로 확인).

**항목 5(테스트 집계) — 정정.** "4,486→4,503, 신규 16건+원인불명 1건"은
잘못된 서술이었다. `unittest.TestLoader().discover()` 순수 수집(실행 아님)
결과, 같은 커밋(e8442bf)의 코드 기준 수집 개수는 4,487건이었다(Phase 1
회귀 실행 로그의 "4,486"과 다름). 4,487+16(diff로 확정된 신규 테스트
정확한 개수, 제거·개명 0건)=4,503으로 정확히 일치한다 — 코드에는 문제가
없다. Phase 1 실행 로그가 왜 4,486이라는 1개 적은 수치를 보고했는지는
그 프로세스가 이미 종료돼 사후 재현이 불가능하므로 확인 불충분으로 남긴다.

상세 근거는
`docs/audits/20260917_APPROVAL_SCOPE_VIOLATION_AND_ORDER_COLLECTION_STRUCTURE.md`
18~22절, Phase 7B 호출 예산은
`docs/HOMEZ_PHASE7B_REAL_TEST_ORDER_PLAN_20260917.md` 3-1절(연결 1개 기준
최초수집 최소1·최대100 + 중복재조회 최소1·최대100 = 합계 최소2·최대200).

이번 라운드에서도 실제 쿠팡 API·실제 주문·발주·결제는 전혀 실행하지
않았다. Phase 7B는 별도의 명시적 승인 전까지 진행하지 않는다.
...
## 2026-09-15 후속 26 — Phase 9A~9K 완료: 안전 기능 10개 구현 +
8-16 재감사

[GitHub 기준 확인] HEAD=origin/main=`f37a0925480c800df8f2762c41fd2550061aeaf2`
(작업 시작 시점, 변동없음). 상세 근거·판정은
`docs/audits/20260914_FULL_AUDIT.md`(13.11~13.14절)와
`docs/HOMEZ_USER_OPERATION_SETTINGS_AUDIT_20260909.md`(개별 행 +
Phase 11 재계산 요약표)가 원본이다 — 이 절은 요약만 남긴다.

이전 세션(Phase 9, 8-16만 구현)이 미착수로 남긴 10개 안전 기능을
전부 착수했다: 7-8(호출제한·캐시), 7-11(매입처 사건 자동
일시중지), 7-16(휴면 공급처 재점검), 8-5/8-6(가격·재고 TTL),
8-19(가상재고 0 제안), 10-4(상품 속성 비교 차단), 10-5(비교 UI),
10-17/10-18(리콜·판매중지 확인 및 차단). 신규 도메인 2개
(`app/domains/product_attribute_match`, `app/domains/recall_notice`)
+ 신규 서비스 1개(`supplier_incident_service.py`) + 신규 Migration
5개(`20260915_05`~`09`, 전부 임시 SQLite에서만 검증) + 신규
알림 이벤트 3개(`wired=True`) + 콘솔 신규 화면 2쌍(목록→상세).

**최종 판정**: 7개 IMPLEMENTED(7-8/7-11/8-5/8-6/8-16/8-19/10-4),
3개 PARTIALLY_IMPLEMENTED(7-16/10-5/10-18), 10-17은 외부 리콜
데이터 출처가 아직 선정되지 않아 PARTIALLY_IMPLEMENTED로 남는다
— "10개 전부 완료"가 아니라 정확히 이 배분이다.

**Phase 9K(8-16 재감사)**: 이미 구현된 항목의 연결별 독립성,
회사간 격리, 재시작 후 지속성을 각각 전용 회귀로 추가 검증했다
— 결함은 발견되지 않았고, 그 사실을 회귀로 고정했다.

**전면 회귀(Phase 10B, 최종)**: `test_purchase_*.py` 전체(429/429),
새 도메인별 전체(price_stock_safety 37/37, product_attribute_match
25/25, recall_notice 29/29, notification 73/73, automation_safety
15/15, scheduler 22/22), `test_notification_delivery_pt3.py`
35/35 — 그리고 **`python -m unittest discover -s tests -p
"test_*.py"` 전체 1회 실행: 4,411건, 실패 2건, 오류 0건, skip 7건
(의도된 real-credential skip)**. 실패 2건 모두 원인 규명·수정 후
집중 재검증 통과(상세: `docs/audits/20260914_FULL_AUDIT.md`
13.12절) — (1) 신규 Migration 5개가 하드코딩된 목록 테스트에
반영 안 된 것(재발한 유지보수 결함 클래스), (2) `payment_methods`
결제수단 테스트가 무작위 토큰 참조 문자열을 상대로 짧은 CVC를
부분일치 검사해 생긴 **이 세션과 무관한 기존 flaky 테스트**(우연히
"999"가 무작위 16진수 접미사에 포함된 것을 유출로 오판) — 검사
대상을 실제 자유텍스트 필드로 좁혀 근본 수정했다. 반복적으로
재발한 결함 클래스 2가지(경량 fixture의 신규 테이블 누락, Migration
DDL-diff 정규식이 `VARCHAR(500)` 같은 괄호 타입을 처리 못하던 버그)
도 이번 라운드에서 직접 원인 규명·수정했다 — 전부 완화나 회피가
아니라 근본 수정이다.

**정직하게 남긴 한계**: (1) 매입처 어댑터 계약이 제조사·원산지·
사이즈·수량 필드 자체를 갖고 있지 않아 10-4의 실데이터 자동 연동은
상품명 1개 필드로만 가능하다. (2) 10-5(비교 UI)는 코드는
작성됐지만 이 세션에서 브라우저로 한 번도 렌더링·클릭 검증하지
못했다. (3) 10-17은 공식 리콜 데이터 출처가 여전히 미정이라
