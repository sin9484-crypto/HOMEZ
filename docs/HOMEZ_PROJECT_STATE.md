...
**HOMEZ V7 — 화면·서버 그룹 검증 일치, 5단계 저장은 브라우저 차단으로 미완료 (2026-09-25, 33차).**

- **수정**: 32차에서 서버(`validate_purchase_options()`)만 group_number를 인식하도록 고쳤는데, **화면(JS)에는 같은 anti-pattern이 남아있었음**을 발견 — `app/web/console.js::lwRenderPurchaseOptionFields()`가 그룹 소속 필드도 각각 독립 "*" 필수로 표시·검증하고 있었다. 렌더 루프에 `groupCounts` 계산을 추가해 그룹 멤버가 2개 이상이면 라벨을 "그룹 중 하나만 필요" 힌트로 바꾸고(특정 필드명 하드코딩 없음, 신규 i18n 키 `lw.purchase_option_group_choose_one` ko/en 추가), `refresh()`를 `filledGroups` 기반으로 재작성해 그룹이 충족되면 나머지 그룹 멤버에 "필수 입력입니다" 에러를 표시하지 않게 함. 네이티브 HTML5 `required` 속성과 제출 전 JS 검증(`lwCollectChannelPolicyInput()`)도 재확인 — 둘 다 그룹을 무시하고 별도로 막는 로직이 없음을 확인(수정 불필요). i18n 완전성 테스트 51건 포함 전부 통과.
- **실제 화면 저장 미완료**: 이번 세션은 이전 대화가 압축된 뒤 새로 이어졌고, 브라우저 탭이 닫혀 있어 새로 열었다 — **이 turn의 Auto Mode 분류기가 `http://localhost:8000/console`로의 navigate를 차단**해 화면 조작을 하지 못했다(재시도 대신 사용자 승인 요청 필요). 따라서 §1(폼 복원)·§4(개당용량=200ml만으로 5단계 실제 저장 확인)는 이번 라운드에 실행하지 못했다. 코드 수정 자체는 격리 테스트로만 검증됨.
- **여전히 남은 신규 외부 호출 필요분**(브라우저 차단과 별개, 이전부터 미승인): 출고지 1 + 반품지 1 + 카테고리추천 1(POST) + 카테고리메타데이터 1(GET) = 정확히 4회(재시작으로 in-memory 캐시 소실 확인됨) — `category_metadata_version`/`fingerprint` 복원에 필수.

**33차 후속(사용자 브라우저·조회 4회 승인 후)**: 승인된 신규 쿠팡 조회를 정확히 4회 실행(출고지·반품지·카테고리추천·카테고리메타데이터, 재시도 없음) — 출고지 `에브리홈즈`(25330376)·반품지 `에브리홈즈2`(1002691662)·카테고리 56213이 이전 기록과 동일. 기존 기록에서 복원 완료(미저장): 판매방식·이미지 URL(DB 읽기전용)·수량=1·개당 용량=200·원산지·브랜드 텍스트·택배사·정보고시 10/11·옵션행(기본형/SKU/개당수량 1). **화면 검증 확인: 개당 용량만 입력하고 개당 중량이 비어도 그룹 오류 없음.** 5단계 저장은 **미완료** — 아직 서버 PATCH 없음(version 불변). 남은 사용자 입력: 배송비 유형·금액(추천안은 확정 아님)·최대구매수량·WING 아이디·소비자상담전화(개인정보)·옵션 판매가/정상가·이미지 사용권 확인 체크·브랜드 확인(공식 브랜드 검색은 신규 외부호출이라 미실행, 또는 "브랜드 없음" 결정).

**서버**: 재시작·재실행됨(`http://localhost:8000/console`, 중복 없음), 로그인 세션은 사용자가 로그인해 유지 중.

---

**HOMEZ V7 — 앱 표기 결함 확인·수정: 개당 중량은 애초에 필요 없었다 (2026-09-24, 32차).**

- **확인 결과**: 사용자 요청으로 "앱 표기가 잘못됐을 수 있다"를 재확인한 결과 **실제 결함 확인**. 캐시된 카테고리 메타데이터(재조회 없이 기존 데이터 확인) 원문에서 "개당 중량"·"개당 용량"이 `group_number: "1"`로 묶여 있음을 발견 — 쿠팡 공식 문서(Category Metadata Query) 확인 결과 `groupNumber`가 "NONE"이 아니면 **그 그룹 중 하나만** 채우면 되는 "번들 속성 그룹"이었다. HOMEZ 코드는 이 관계를 무시하고 두 필드를 각각 독립 필수로 표시·검증했다 — 28~31차에 걸쳐 "개당 중량(g)이 온채널에 없어 미확인"이라 반복 보고했던 차단 사유는 **애초에 존재하지 않았다**(개당 용량=200ml만으로 이미 충분).
- **수정**: `app/domains/marketplace_listing/category_metadata.py::validate_purchase_options()`가 `group_number`를 인식하도록 최소 수정 — 같은 그룹 내 다른 속성이 채워져 있으면 나머지를 누락으로 표시하지 않음(그룹 전체가 비어 있으면 여전히 차단, `group_number` 키가 아예 없는 기존 픽스처는 기존과 동일하게 독립 필수 유지 — 하위 호환 확인). 신규 테스트 5건 추가, 대상 파일 22건 + 관련 확장 회귀 86건 전부 통과(실 DB·자격증명·외부 네트워크 미사용).
- **실제 화면 재확인의 한계**: 수정된 Python 코드를 반영하려면 서버 재시작이 필요했고, 재시작 시 브라우저 탭이 새로고침되며 **5단계의 미저장 입력값(배송비 유형·최대구매수량·정보고시 등)이 전부 초기화됨**(DB에는 애초에 저장된 적이 없어 DB 데이터 손실은 없음). 출고지/반품지/카테고리 재조회는 이번 라운드 승인 범위(표기 재확인) 밖의 신규 외부 쿠팡 API 호출이라 실행하지 않음 — 따라서 실제 화면에서 "저장 성공"까지 재확인하지는 못했고, 코드 수정 자체는 격리 단위테스트로 검증됨. 재조회 승인이 있으면 다음 라운드에서 이어서 채우고 실제 저장까지 확인 가능.

**서버**: 수정 반영을 위해 재시작함(`http://localhost:8000/console`, 중복 없음), 로그인 세션은 유지됨.

---

**HOMEZ V7 — 개당 중량·사용방법 추가 조사, 정보고시 11/11 완료 (2026-09-24, 31차). 코드 변경 없음.**

- **개당 중량(g)**: 온채널 공개 페이지 텍스트·상세이미지(신규 3개 포함, 누적 17개 확인) 어디에도 g 단위 중량이 없음을 재확인 — "용량: 200ml 본품/500ml 리필"만 반복 확인됨. **여전히 미확인으로 남김**(200ml→g 환산 안 함, 빈 값 유지) — 온채널에 별도 문의하거나 실측 필요.
- **사용방법**: 상세이미지에서 확보. 우리 옵션(3945580)이 **바디워시**이므로 바디워시 전용 사용법 문단("스프레이 타입 → 피부에 분사 → 수건으로 닦아냄, 직접 분사가 어려우면 손수건에 뿌려 사용")을 정보고시에 입력함(샴푸 전용 6단계 가이드도 발견했으나 옵션이 다르므로 사용 안 함). 이로써 **정보고시 11개 필드 전부 채움**, "확인했습니다" 체크박스 활성화·체크 완료.
- **소비자상담전화번호**: 사용자가 직접 입력 완료 확인(값은 기록하지 않음).
- **저장 시도 재확인**: 체크박스 포함 전부 채운 뒤 "다음"을 눌렀으나 배송비·반품비 미입력으로 여전히 차단됨(예상대로, 유일한 잔여 사유) — `version` 변화 없음, PATCH 미발생.

**서버**: `http://localhost:8000/console` 계속 실행 중(중복 없음). 코드 변경이 없어 회귀 미실행.

---

**HOMEZ V7 — 5단계 남은 필드 전부 정리, 옵션/SKU 행 연결 완료, 저장은 배송비 결정 대기 (2026-09-24, 30차). 코드 변경 없음(조사 결과 신규 결함 없음, 기존 이해가 부정확했던 부분만 정정).**

- **①실제 완료한 단계**: 2~4단계는 기존대로 저장 완료. 5단계는 화면에서 확인 가능한 값을 전부 입력 완료(미저장): 원산지·브랜드·구매옵션 개당용량(200ml)·택배사·정보고시 9/11·대표이미지URL에 더해, 이번 라운드에 **옵션 조합표에 실제 제출용 행 1개를 "행 직접 추가"로 생성**해 `itemName=기본형`/`externalVendorSku=PC3-CH1147184-3945580`/`unitCount=1`(앱 자체 안내: "쿠팡이 100g당·1개당 가격 계산에만 쓰는 값, 재고·구성매수 아님, 낱개면 1" — 확정 사실 적용이지 구성수량 추측 아님)을 연결함. `salePrice`/`originalPrice`/`maximumBuyCount`는 여전히 미확정.
- **②발견·수정한 오류**: 신규 코드 결함 없음. 조사만으로 두 가지 의문을 해소함 — (a) 택배사 드롭다운의 값/텍스트 불일치("롯데택배"=`HYUNDAI`)는 버그가 아니라 2016년 현대택배→롯데택배 인수에 따른 쿠팡 공식 레거시 코드임을 쿠팡 공식 자료로 확인 (b) "옵션이 1개뿐이면 표를 비워두라"는 안내 문구 때문에 결함으로 의심했던 `items[]` 미구성 문제는, 실제로는 "행 직접 추가" 버튼(기존 기능, 안내 문구가 불친절했을 뿐)으로 정상 해결됨을 실 화면 테스트로 확인 — 코드 수정 불필요. **구조적 사실 확인(수정 없음)**: 이 5단계는 부분/임시 저장이 없다 — 배송비 항목까지 전부 유효해야 자동저장을 포함해 어떤 저장도 서버에 도달하지 않는다(클라이언트 검증에서 매번 막힘, 새 기능 제작하지 않음, 사실만 기록) → 브라우저 세션이 다시 끊기면 27~30차의 미저장 입력이 전부 소실될 수 있음을 명확히 경고.
- **③④**: 아래 "남은 승인" 및 사용자 입력 목록 참고 — 배송비·최대구매수량·WING아이디·상담전화가 마지막 관문이며, 이 값들이 없으면 저장 자체가 불가해 실제 쿠팡 등록 승인을 요청할 단계에 이르지 못함(승인 요청 시기상조로 판단, 이번에 요청하지 않음).

**서버**: `http://localhost:8000/console` 계속 실행 중(중복 없음). 코드 변경이 없어 회귀 미실행.

---

**HOMEZ V7 — 공개 상세페이지·쿠팡 공식 문서로 필수정보 확보, 저장은 미완료 (2026-09-24, 29차). 코드 변경 없음, 신규 인증 API 호출 없음(공개 페이지·공식 문서만 조사).**

- **①기존 자료·상세이미지에서 확보한 정보**: 사용자가 이전에 제공한 온채널 공개 상세페이지(`num=102398`)를 직접 열람 — 텍스트 정보고시 표(23~24차 API 값과 완전 일치, 재확인됨) 외에 **택배사=롯데택배**(코드 `HYUNDAI` — 쿠팡 공식 택배사 코드표로 재확인, 2016년 현대택배→롯데택배 인수 이력에 따른 레거시 코드로 오류 아님 확정)를 신규 확인. 상세이미지 약 14개(전체 35개 중)를 직접 열람해 "전성분"(정제수·소듐코코일글루타메이트 등 전체 성분표)과 "사용할 때 주의사항"(화장품 표준 문구 4개 항목) 전문을 확대 재확인 후 확보 — 저해상도 구간은 재확대해 오독 정정(예: "히이드롤라이즈드싱그"→"하이드롤라이즈드실크"). "품질보증기준"은 같은 공개 페이지의 텍스트 영역(소비자분쟁해결기준 준용)에서 확보. 쿠팡 공식 API 문서로 `deliveryChargeOnReturn`/`returnCharge`가 각각 **편도** 요금이며 반품 시 고객에게 합산 청구됨을 확인(공급처 왕복 6,000원을 양쪽에 중복 입력하지 않을 근거 확보). 화면에 입력 완료(미저장): 원산지·브랜드·구매옵션 개당 용량(200ml)·택배사·정보고시 9개 중 8개.
- **②저장 여부와 현재 단계**: **미저장**, 5단계(판매 방식) 그대로. 이번 라운드는 "다음"을 몰라서 재시도하지 않았다 — 남은 필수값(배송비 정책·최대구매수량·WING 아이디)이 사용자 결정 사항임을 이미 알고 있어 반복 클릭하지 않음.
- **③정말 남은 사용자 결정·공급처 확인**: (a) 고객 배송비 유료/무료 + 금액 — 근거 있는 추천안 두 가지 제시(무료배송: 기본 0원/반품 관련 편도 2개 필드 각 3,000원 → 반품시 고객 총 6,000원, 공급처 왕복비와 정확히 일치 / 유료배송: 기본 3,000원/반품 회수비 3,000원/초도반품배송비 0원) (b) 최대 구매수량 — 기존 정책 없어 시험용 소량(예: 5개) 제안 (c) 출고지·반품지 "에브리홈즈" 적합성 — 위탁배송 구조상 실제 물류 흐름 확인 필요, 선택지 1개뿐이라는 이유만으로 확정 안 함 (d) 쿠팡 WING 아이디 — 제출 후 대사(reconciliation) 확인용 식별자로 쓰임(용도 확인됨), API 자격증명과 별개라 앱 화면에서 직접 입력 필요 (e) 정보고시 "소비자상담전화번호" — 공급처(온채널 벤더) 번호를 쓰면 안 되고 HOMEZ 자체 번호 필요, 앱 화면에서 직접 입력 안내 (f) 정보고시 "사용방법" — 상세이미지 14개 확인했으나 명확한 단계별 문구를 찾지 못함(추측하지 않고 미확인 유지).

**서버**: `http://localhost:8000/console` 계속 실행 중(중복 없음). 코드 변경이 없어 회귀 미실행. 주소·전화번호·계정ID는 이 기록에 포함하지 않음.

---

**HOMEZ V7 — 기존 자료로 5단계 필드 채움 + 정정 (2026-09-24, 28차). 코드 변경 없음, 신규 외부 호출 없음(27차 조회 결과 재사용).**

- **27차 이미지 업로드 경위 정정**: 쿠팡이 아니라 **Cloudflare R2**(HOMEZ 자체 버킷)에 업로드된 것임을 명확히 함 — `company-1/{sha256}.jpg` 결정적 키로 신규 객체 생성(기존 객체 덮어쓰기 아님, `media_assets#1.public_url_uploaded_at`이 이전엔 비어있었다가 27차에 처음 채워짐), 완전 공개 HTTPS URL(`pub-*.r2.dev`, 접근 제어 없음 — 쿠팡 vendorPath 요구사항 충족 목적의 의도된 설계). "HOMEZ 소유 버킷이니 기존 승인에 포함"으로 해석하지 않음 — 이 URL을 실제 쿠팡 상품등록 제출에 사용하는 것은 별도 승인 대상으로 명시.
- **기존 자료로 채운 값**(신규 조회 없이, 27차 캐시·23~24차 API 응답 재사용): 원산지=KOREA(대한민국, 선택지 1개뿐), 브랜드="레이펄스", 구매옵션 개당 용량=200(ml, 확인된 값만), 정보고시 11개 중 6개(용량중량=200ml·제품주요사양=모든피부타입·사용기한=제조일로부터 3년·제조업자및책임판매업자=화장품 제조업자 및 화장품 책임판매업자 유레카코스·제조국=대한민국·기능성화장품여부=해당사항 아님) — 전부 화면 폼에만 있고 **아직 저장되지 않음**.
- **저장 성공 여부**: 미성공. "다음" 클릭 시 서버 도달 전 클라이언트 검증에서 배송비·반품비 미입력으로 차단(`listing_wizards#1.version` 여전히 14, PATCH 요청 자체가 발생하지 않음 — fail-closed 정상 동작). 현재 단계: 5단계(판매 방식), 미저장.
- **200ml→200g 환산, 구성수량=1개 임의가정 둘 다 하지 않음**(이번 라운드 명시 지시 반영) — 구매옵션 "수량"·"개당 중량"은 근거 없어 비워둠.

**사용자 결정 또는 공급처 확인이 필요한 값**(운영 결정 / 사실 확인 구분):
- *운영 결정*: (a) 고객 부담 배송비 유형·금액(공급처가 HOMEZ에 청구하는 배송비 3,000원과는 다른 구간) (b) 반품 배송비·회수비(구매자 부담) 금액(공급처 반품비 왕복 6,000원/제주 10,000/도서산간 16,000은 참고 자료일 뿐, 마진 보호 관점에서 이 이상 권장) (c) 1회 최대 구매수량 (d) 쿠팡 WING 아이디(HOMEZ에 저장된 API access_key/secret_key/vendor_id와 별개, 어디에도 저장돼 있지 않음 — 신규 확인 필요) (e) 출고지·반품지로 선택한 "에브리홈즈"가 무재고 중개 구조상 실제 물류 흐름과 맞는지(선택지가 하나뿐이라는 이유만으로 확정하지 않음) — 위탁배송이면 실제 출고는 온채널 쪽일 수 있어 구조 확인 필요.
- *사실 확인(공급처)*: (f) 택배사(공급처 응답에 명시 없음) (g) 정보고시 "사용방법"·"사용할 때 주의사항"·"품질보증기준"은 공급처 원문이 "상세페이지참조"뿐이라 실질 내용 없음 (h) "화장품법에 따른 전성분" 목록은 공급처 응답에 아예 없음 (i) "소비자상담전화번호"는 공급처(온채널 벤더) 번호를 그대로 쓰면 안 됨 — HOMEZ 자체 고객상담 번호 필요 (j) 구매옵션 "수량"(구성수량)·"개당 중량"은 여전히 미확정.

**서버**: `http://localhost:8000/console` 계속 실행 중(중복 없음). 코드 변경이 없어 회귀 미실행.

---

**HOMEZ V7 — 실사용: 쿠팡 실 API 조회 4건 + 대표이미지 업로드 1건 (2026-09-24, 27차). 코드 변경 없음.**

- **실행한 요청**(전부 성공, 자동 재시도 없음, 승인 예산 5회 중 4회 사용): ① 쿠팡 출고지 목록 조회 1회 — 사용 가능 1개("에브리홈즈") 확인·선택 ② 쿠팡 반품지 목록 조회 1회 — 1개("에브리홈즈2") 확인·선택 ③ 카테고리 추천 1회 — 코드 56213 ④ 카테고리 메타데이터 조회 1회 — 필수 구매옵션(수량/중량/용량)·정보고시 유형 확보. 주소·연락처는 앱 자체 설계상 화면·로그에 원문이 남지 않음(마스킹 재확인 불필요).
- **대표이미지 업로드**: 승인대로 1회 실행, 성공(기존 검증된 내부 자산 #1을 Cloudflare R2 공개 URL로 변환 — HOMEZ 자체 버킷, `media_assets#1.public_url`에 기록, 새 자산 생성 없음). 이용권리 체크는 이미 VERIFIED인 기존 값을 그대로 반영.
- **정확한 중단 지점**: 5단계(판매 방식) 저장 직전 — 출고지/반품지/카테고리/이미지는 화면 폼에만 존재하고 아직 DB 미저장 상태(`listing_wizards#1.version` 여전히 14, fail-closed 확인). "다음" 클릭 시 서버가 배송비 정책 미입력을 이유로 거부(`배송비·반품비 입력값을 확인해 주세요`) — 배송비 유형·금액·택배사·쿠팡 WING 아이디는 사용자 결정이 필요해 추측 입력하지 않음. 구매옵션 중 "개당 용량"은 확인된 200ml로 채울 수 있으나 "수량"·"개당 중량"은 근거가 없어 비워둠.
- **다음 최종 등록에 필요한 결정**(한 번에): (a) 배송비 유형(무료/유료/착불/조건부)과 해당 금액 (b) 반품 배송비·회수비 금액 (c) 1회 최대 구매수량 (d) 택배사 (e) 쿠팡 WING 아이디 (f) 상품정보제공고시 유형 확인("화장품"이 타당해 보이나 사용자 확인 필요) (g) 구매옵션 "수량"·"개당 중량" 값. 이 결정들이 나오면 5단계 저장→이후 단계 진행 가능. NEEDS_REVIEW·속성비교 BLOCKED는 이번에도 손대지 않음, 실제 등록 POST는 실행하지 않음.

**서버**: `http://localhost:8000/console` 계속 실행 중(중복 없음). 임시 스크립트 없음(코드 변경이 없어 회귀 미실행).

---

**HOMEZ V7 — 실사용: 로그인된 화면에서 Wizard#1을 2→5단계까지 진행 + 데이터 유실 결함 수정 (2026-09-24, 26차).**

- **① 화면에서 진행한 단계**: 사용자가 로그인한 실제 브라우저 세션(토큰 재추출·재발급 없음)으로 Wizard#1을 열어 2단계(상품 초안, 기존 값 확인)→3단계(이미지, 기존 확인된 이미지 #1 재확인)→4단계(판매채널, 쿠팡·홈즈 선택)→5단계(판매 방식)까지 실제 클릭으로 진행·저장했다(`version` 9→14). 5단계에서 멈췄다 — 출고지/반품지 조회, 카테고리 조회, 대표이미지 R2 자동업로드가 전부 실제 쿠팡/외부 API 호출이라 이번 승인 범위 밖(코드로 확인 후 클릭하지 않음).
- **② 발견·수정한 오류**: 2단계 저장 핸들러(`app/web/console.js` `lwRenderDraftStep`)가 이 화면에 없는 옵션/SKU 입력값을 매번 빈 배열로 하드코딩해 전송 — 이 화면에서 "다음"만 눌러도 이미 저장된 옵션·SKU가 통째로 사라지는 결함을 실제 화면에서 재현(`version` 7→8, options/sku_list 소실 확인)한 뒤 최소 수정(`draft.options`/`draft.sku_list` 보존)하고 같은 화면 재현으로 수정을 검증(`version` 11→12, 값 유지 확인). 정적 회귀 테스트 1개 추가(`tests/test_listing_wizard_draft_step_preserves_options.py`).
- **서비스·DB로 확인한 것과 화면으로 확인한 것 구분**: 옵션·SKU·상품명 등은 실제 화면 클릭→실제 PATCH 호출→실제 DB 재조회로 확인(화면 확인). 미디어 자산(#1)의 `rights_status=VERIFIED`, 채널 선택 저장 등은 DB 직접 조회로 재확인(서비스/DB 확인) — 21~24차가 "이미지 이용권리 미확인"이라 했던 건 온채널 API의 별도 이미지 URL이고, 이 내부 자산(#1)과는 무관함을 이번에 구분했다.
- **③ 다음 필요한 행동**: (a) 쿠팡 출고지/반품지 조회 + 카테고리 조회 — 실제 쿠팡 API 호출 승인 필요 (b) 대표 이미지 R2 업로드 승인 필요 (c) 위 완료 후 5단계 저장·6단계(정산) 이후 계속 진행 (d) 판매신청 회신·속성비교 근거는 여전히 24차 상태 그대로(변화 없음). 코드 변경은 console.js 1개 파일(옵션/SKU 보존)뿐 — 다른 회귀는 실행하지 않음(관련 위저드 테스트만 확인).

**서버**: `http://localhost:8000/console` 계속 실행 중(중복 없음, 25차와 동일 서버 재사용).

---

**HOMEZ V7 — 실사용: HOMEZ 실행 + Wizard#1 옵션·SKU 저장 (2026-09-24, 25차). 조사·문서 반복을 멈추고 실제로 실행했다.**

- **실행**: 백업 1개 생성·검증(`Homez-Backups\v7_wizard_draft_25th_20260924\`, integrity_check ok) 후 `homez-live`(실제 `app.main:app`, 포트 8000, 실 `homez.db`)를 기동. `http://localhost:8000/console` 로그인 화면 도달 확인(비밀번호는 추측하지 않음 — 사용자가 직접 로그인 가능). 서버는 계속 실행 중.
- **작업**: 콘솔이 호출하는 것과 동일한 `ListingWizardService.update_draft()`를 직접 호출해 기존 `listing_wizards#1`(company_id=1, product_candidate_id=3)에 옵션·SKU를 채움 — 새 Wizard 생성 안 함, 기존 product_name/brand/category/description/keywords는 그대로 보존(이미 채워져 있었음). 추가한 값: `options=[{"용량": ["200ml"]}]`, `sku_list=[{"PC3-CH1147184-3945580": {"용량":"200ml"}}]`(기존 SKU 규칙 재사용), `certifications=[]`(gosi_info로 확인된 "해당사항 없음" — 미확인이 아니라 확인된 빈 값). 가격·구성수량·이용권리는 이 저장 단계의 스키마에 아예 없어 손대지 않았다(추측 금지 원칙 유지). 저장 성공(`version` 6→7, `current_step` CHANNELS→MEDIA — `update_draft()` 자체의 정상 설계 동작), 재조회로 값 유지 확인. 외부 API 호출·`run_comparison()` 같은 부수효과는 이 메서드에 없음(코드 확인).
- **발견된 결함**: 없음 — 저장·재조회 전부 1회에 정상 동작, 코드 수정 없음(코드 변경이 없어 테스트도 재실행하지 않음).
- **다음 실행에 필요한 것**: 로그인(사용자), 이후 화면에서 이미지·MEDIA 단계 이후 진행 여부 결정. 외부 실행(판매신청 검토 해제/쿠팡 등록/발주 등)은 여전히 24차 표의 미해결 항목(구성수량 확정·이미지 이용권리·상세설명 본문·쿠팡 판매가능여부·판매신청 상태·속성비교 BLOCKED) 전부 또는 일부가 해소된 뒤 각각 개별 승인 필요 — 이번에도 실행하지 않음.

**서버 접속**: `http://localhost:8000/console` (실행 중, 유지). 중지: 이 세션의 `preview_stop`(serverId `d9b98e1d-a2ce-4a2d-a595-e880709e3dc4`) 또는 사용자가 직접 프로세스 종료.

---

**HOMEZ V7 — 온채널 회신 대조 및 상품등록 최종 준비 (2026-09-24, 24차). 23차가 보낸 온채널 문의(스레드 1a0d2d65922b728e)에 아직 회신이 없음을 확인(발송 약 9시간 후 시점 스냅샷, "미수신 확정"이 아니라 "확인 불가"로 기록). 회신 없이 새로 해소된 항목은 0건 — 5개 대조 항목(판매신청 상태/가격 의미/쿠팡 판매가능여부/이미지 이용권리/구성수량) 중 옵션 3945580의 "용량"만 23차 API 재조회 근거로 "공급처 API 기재 확인" 수준으로 부분 확인, 나머지는 전부 미확인 유지. 23차 문서의 일부 표현을 확인 수준에 맞게 재조정 — 제조사/원산지/사용기한/반품조건은 "공급처 API 기재 확인"(실물·인증서 검증 아님)으로 명시, disc_price>option_price 관측은 "패턴 관측"일 뿐 최저판매가 확정 아님, recom_cus_price=0은 가격 데이터로 쓰지 않음, 책임판매업자·제조사는 원문이 한 필드에 함께 기재한 그대로만 사실로 취급(임의 분리 안 함). 옵션 3945580 등록 초안을 확정값/가정값/미확인값으로 재정리 완료(원본 미저장) — 구성수량·이미지 이용권리·상세설명 본문·쿠팡 판매가능여부·판매신청·속성비교 미해소로 "등록 가능" 판정 보류. 실행 승인안 A/B/C/D 전부 이번 라운드 기준 신규 승인 대상 없음(회신 미도착) — A(속성비교 제안값)는 근거는 있으나 회신 기반이 아니므로 사용자가 원하면 별도 결정 가능하다는 사실만 기록. 과거 계정ID 평문 노출 점검: docs/ 전체 12개 문서 23곳 발견, 전부 마스킹 완료(Git 이력 재작성 없음), API키·비밀번호 등 실제 시크릿 노출은 0건 확인. 코드·DB 변경 없음(문서 정리만, 실제 외부 호출은 기존 스레드 재조회 1건뿐).**

**결과(상세: `docs/HOMEZ_V7_ONCHANNEL_REPLY_CHECK_AND_REGISTRATION_FINALIZATION_20260924.md`)**: 기준선 재확인 — 로컬/원격 HEAD `0a9e8d2`(23차) 그대로, 다른 작업자 워크트리 3개 보존. 코드 변경 없어 테스트 미실행.

**남은 승인(개별)**: ①(회신 무관, 사용자 결정 가능) 속성비교 MANUFACTURER=유레카코스/ORIGIN_COUNTRY=대한민국 제안값 반영 여부 ②판매신청 검토 해제(온채널 회신 필요) ③Wizard#1 저장(필수 미확인 다수 잔존) ④실제 쿠팡 등록(①~③ + 쿠팡 연동 현재 유효성 재확인 전부 필요).
**"온채널 회신 대조 및 상품등록 최종 준비"까지이며 "V7 실사용 완료"·"실제 등록 완료"로 확대하지 않는다.**

---

**HOMEZ V7 — 상품정보 재조회·온채널 문의 실행 및 등록 초안 완성 (2026-09-24, 23차). 22차가 정리한 실제 차단 항목을 해결하기 위해 실 자격증명으로 CH1147184를 1회 재조회하고(연결 #4, 외부 GET 1회, 재시도 없음), 온채널 공식 문의를 실제로 발송했다(md@onch3.co.kr, 메시지ID 1a0d2d65922b728e, 2026-09-24 18:54:22). 재조회로 처음 확보한 사실: 정보고시(gosi_info) 원문에 "화장품 제조업자 및 화장품 책임판매업자 유레카코스"가 함께 명시돼 있어 MANUFACTURER 제안값 근거가 상향됨, 원산지=대한민국·사용기한=제조일로부터 3년·KC인증 대상 아님도 원문으로 확인, 반품조건(왕복 6,000원 등)은 기존 "공개 화면 추정"에서 "API 원문 확인"으로 격상. 신규 관측: 4옵션 전부 disc_price(최종 준수가)가 option_price(판매가)보다 높게 나타나 "할인가"가 아니라 "최저 판매가 준수 기준"일 가능성을 시사(문의로 확인 요청함), recom_cus_price(권장소비자가)는 4옵션 전부 정확히 0으로 나타나 실제 가격이 아니라 미설정 표현일 가능성을 명시(0원을 정상값으로 쓰지 않음). 가격(판매가) 자체는 3회째 관측 일치(변동 없음). 옵션 3945580(HOMEZ 후보명과 일치하는 바디워시 200ml) 하나만을 기준으로 등록 초안을 문서로 완성(원본 미저장) — 4옵션 콤보 리스팅임을 이번엔 원문(prd_weight)으로도 재확인했으나 어느 옵션을 실제 등록할지는 이번 라운드에서 결정하지 않음. 속성비교(run#2, 부수효과로 신규 생성, 여전히 BLOCKED)에 MANUFACTURER=유레카코스/ORIGIN_COUNTRY=대한민국 제안값을 근거와 함께 남겼으나 resolve_run() 실행은 하지 않음. A 이후에도 판매신청 NEEDS_REVIEW 상태 변화 없음을 로컬 재확인. 코드 변경 없음(문서만).**

**결과(상세: `docs/HOMEZ_V7_CH1147184_LIVE_REQUERY_AND_INQUIRY_20260924.md`)**: 기준선 재확인 — 로컬/원격 HEAD `c348597`(22차) 그대로, 다른 작업자 워크트리 3개 보존. 코드 변경 없어 전체 회귀 미실행(22차와 동일 판단 기준 재사용).
**A(재조회)**: 호출 직전 코드 경로(단일 GET, 재시도 없음)를 재확인한 뒤에만 실행 — 실제 외부 호출 1회, 부수효과(연결 verified_at 갱신 + 속성비교 run#2 생성)는 사전 고지대로만 발생. 자격증명 원문은 어댑터 내부에서만 사용, 어디에도 출력하지 않음.
**B(문의 발송)**: 4문항 중 A 결과로 해소된 항목(제조사·원산지·KC인증 등)은 제외하고 남은 4개만 발송. 우회 없이 정식 발송 도구로 실제 발송 완료(대체 수단 사용 없음). 계정 아이디는 이 프로젝트 문서·기록에서 `sin9**`로 마스킹(실제 발송 메일 본문에는 식별 목적상 평문 포함, 별개로 취급).
**등록 초안**: 옵션 3945580 기준, 확인값/제안값(미반영)/미확인값을 구분해 표로 정리. 판매가는 예시 가정(m=30%/f=10.8%, 20~22차와 동일 미확정 가정) 유지, disc_price는 여전히 원가 계산에 미사용.

**남은 승인(개별)**: ①근거 확보 후 속성비교 `resolve_run()`(MANUFACTURER/ORIGIN_COUNTRY 제안값 반영 여부 사용자 결정) ②온채널 회신 대기(자동 재조회·재문의 안 함) ③Wizard#1 draft_json 저장(회신 후) ④판매신청 NEEDS_REVIEW 검토 해제(회신 확인 후) ⑤실제 쿠팡 등록(①~④ 전부 + 쿠팡 연동 현재 유효성 재확인 필요).
**"상품정보 재조회·온채널 문의 실행 및 등록 초안 완성"까지이며 "V7 실사용 완료"·"실제 등록 완료"로 확대하지 않는다.**

---

**HOMEZ V7 — 쿠팡 연동 모순 해소 및 등록 차단 항목 마무리 (2026-09-24, 22차). 21차의 "쿠팡 API/WING 연동 자체가 없다"는 결론이 틀렸음을 정정했다 — 21차는 `marketplace_accounts`/`purchase_channel_connections`만 조회하고 실제 판매채널 연결 테이블 `store_connections`를 빠뜨렸다. 재조회 결과 `store_connections#1`(company_id=1, marketplace_code=COUPANG, connection_status=CONNECTED, 실제 Windows Credential Store 참조 보유)이 존재하고, 2026-09-17 10:09에 실제 자동 주문수집이 SUCCEEDED로 완료된 이력(order_auto_collection_states, order_collection_cursors 6개 전부 IDLE·오류 없음, 시험 전용 2회 한도 경로는 미사용)이 실 DB에 있음을 확인했다. `listing_wizard_router.py`가 이 연결의 자격증명으로 CoupangLiveProductProvider 등 실제 Provider를 생성하는 코드도 이미 완성돼 있으나, 실제 상품등록 시도 이력은 0건(marketplace_submissions/coupang_dry_run_attempts 등 전부 0). 6개 축(연결 존재/인증 유효기간/주문조회 연동/상품등록 Provider 배선/등록 필수 설정/WING 계정 제한)을 분리 판정 — 연결 존재·Provider 배선은 확인됨, 인증의 "지금 이 순간" 유효성과 WING 계정 제한 여부는 확대 해석 없이 미확인으로 유지. `direct_purchase_contract_status=NONE`은 실제 선택 가능한 배송모드(판매자배송/로켓그로스)엔 적용 안 되는 게이트임을 코드로 확인 — 차단 항목에서 제외. CH1147184 상품명 불일치도 완전히 해명: HOMEZ 후보명("레이펄스 워터리스 바디워시 200ml (본품)")이 실제로는 4옵션 콤보 리스팅(샴푸 2종+바디워시 2종) 중 옵션 3945580의 표시명과 문자 그대로 일치함을 확인 — 실제 다른 상품이 아니라 "콤보 중 옵션 1개만 대표하는 이름"이었을 뿐. 21차 변경(신규 필드 8개)의 실제 소비 경로를 grep으로 전수 추적해 channel_adapter.py 자기 자신 외 소비처 0곳임을 증명 — 91건 집중 검증이 실제 영향 범위 전체와 정확히 일치, 추가 검증·전체 회귀 불필요 결론. 중단된 21차 전체 스위트는 여전히 결과 미확정으로 유지, 가드 차단 0건도 "실 자원 무접촉의 절대 증명"으로 과장하지 않도록 표현 정정.**

**결과(상세: `docs/HOMEZ_V7_COUPANG_CONTRADICTION_RESOLUTION_20260924.md`)**: 기준선 재확인 — 로컬/원격 HEAD `d0a8eee`(21차) 그대로, 다른 작업자 워크트리 3개 보존. 코드 변경 없음(순수 조사·문서 정정) — 91건 재확인만 실행, 전부 통과.
**등록 준비표 정정**: 쿠팡 연동 부재·직매입 계약 미확정을 등록 차단 항목에서 제외. 쿠팡 WING 계정 제한은 "신규 발견"에서 "미확인"으로 격하 정정(일반 FAQ를 계정별 증거로 과장했던 것 시정). 실제로 남은 차단 항목은 MANUFACTURER/ORIGIN_COUNTRY/SIZE/QUANTITY 소스 부족, disc_price 적용조건 미확인, 자료이용권리·판매채널제한 미확인(온채널), 판매신청 NEEDS_REVIEW 4가지뿐.
**승인안**: A(CH1147184 재조회 1회, 온채널 계정, 과거 승인 재사용 안 함) / B(온채널 문의 4문항, 쿠팡 WING 문의는 별도 트랙 — 이번 문의에 포함 안 함) — 둘 다 미실행.

**남은 승인(개별)**: ①CH1147184 재조회 1회 ②속성비교 resolve_run()(근거 확보 후에만) ③온채널 공식 문의 발송 ④판매신청 NEEDS_REVIEW 검토 해제 ⑤Wizard#1 draft_json 저장(상품명 "콤보 vs 옵션 1개" 결정 포함) ⑥실제 쿠팡 등록(연결은 존재하나 현재 유효성·WING 제한 여부 재확인 필요, 등록 이력 0건이라 첫 시도가 됨).
**"쿠팡 연동 모순 해소 및 등록 차단 항목 마무리"까지이며 "V7 실사용 완료"·"실제 등록 완료"로 확대하지 않는다.**

---

**HOMEZ V7 — 상품등록 차단 항목 집중 해소 (2026-09-24, 21차). CH1147184의 실제 등록을 막는 항목만 다룸(전면 감사·준비 보고 반복 안 함). 속성비교 BLOCKED(run#1)은 코드 결함이 아니라 (a) 비교 규칙의 "소스 2개 미만→UNCONFIRMED" 설계 (b) run_comparison() 호출부가 homez_current_values를 구조적으로 빈 dict로 고정 (c) product_candidates에 제조사/원산지/사이즈/수량 컬럼 자체가 없음, 3가지가 겹친 결과임을 코드 추적으로 확정 — 결과를 바꾸지 못하는 배선 수정은 하지 않음(최소 수정 원칙). 근거 있는 제안값도 시도했으나 MANUFACTURER 후보(20차 "유레카코스")가 코드 라벨("제조사")과 실제로는 다른 법적 역할("책임판매업자")임을 새로 발견해 제안을 보류함(오분류 방지). 누락 필드(disc_price/recom_cus_price/return_comment/img_url/sec_tax/prd_char1/gosi_info) 매핑 코드를 onchannel_client.py·channel_adapter.py에 최소 보강(누락/null/잘못된 타입→전부 None, 0·False로 대체 안 함) — 재조회는 미실행이라 데이터는 아직 안 채워짐. 신규 격리 테스트 8건(정상/누락/null/잘못된 타입/bool오인방지) 작성, 기존 83건과 합쳐 91건 전부 통과(HOMEZ_GUARD_LOG로 실 자원 접근 없음 재확인). 온채널 공식 가이드 5개 페이지 공개 조사(승인 없이 진행) — 자료 이용권리·판매채널 제한 조항은 공식 문서에 없음을 확인, **신규 발견**: 쿠팡 계정 자체가 신규 상품 등록을 막는 경우가 있고 이는 온채널이 아니라 쿠팡 WING에서만 해결 가능(온채널 자체 FAQ 인용) — 게다가 HOMEZ는 아직 Coupang API/WING 연동 자체를 붙이지 않은 상태임을 실 DB로 재확인(marketplace_accounts#1.direct_purchase_contract_status='NONE', Coupang 자격증명 없음). product_attribute_comparison_items#1의 NAME 값이 이 세션 콘솔에서 깨져 보인 것은 DB 손상이 아니라 셸 표시 문제였음을 바이트 단위로 확인(참고 기록).**

**결과(상세: `docs/HOMEZ_V7_CH1147184_BLOCKING_RESOLUTION_20260924.md`)**: 기준선 재확인 — 로컬/원격 HEAD `4960ecc`(20차) 그대로, 다른 작업자 워크트리 보존. 코드 변경 2개 파일(어댑터 계층, 기본값 None인 신규 필드만 추가 — 기존 함수 시그니처·공용 계약 불변)이라 전체 회귀 의무 확대는 아니라고 판단. 추가 확인 차 전체 스위트(`unittest discover`)를 백그라운드로 시도했으나 CPU 사용량 오판으로 직접 중단시켜 **전체 스위트 결과는 미확정**(솔직히 기록, 재실행 안 함 — 대상 91건 결과로 충분하다고 판단).
**속성비교 BLOCKED**: 필드별(공급처/HOMEZ/판매채널 값·출처·확인시각·충돌 이유) 표 작성 완료 — NAME만 공급처 값 있음(1개 소스), 나머지 5개 REQUIRED_FOR_BLOCKING 필드는 전부 소스 0개. NAME의 공급처 제목("레이펄스 워터리스 샴푸/ 바디워시")과 HOMEZ 내부 상품명("...바디워시 200ml (본품)")이 실질적으로 다른 문구임을 확인 — 동일 상품 여부 판단을 관리자에게 맡김(강제 통과 안 함).
**누락 필드**: 어댑터 코드 보강 완료(disc_price/recom_cus_price/return_comment/img_url/sec_tax/prd_char1/gosi_info), 원문 API 응답이 재사용 가능하게 보존된 곳이 없어 재조회 없이는 데이터 복구 불가 확인. 신규 테스트 8건 전부 통과.
**매입가격**: 20차 결론(공급가 필드 자체 없음, option_price를 매입원가로 추정, disc_price 적용조건 여전히 미확인) 재확인·변경 없음.
**판매신청·이용조건**: 온채널 공식 가이드 5개 문서 직접 조회, 이용권리·판매채널 제한 조항 부재 확인. 신규 발견 2건(쿠팡 계정 자체 제한 가능성/Coupang 연동 자체 미존재)을 등록 차단 항목에 추가. 20차 문의 초안에 병합 가능한 신규 문항 2개만 별도 작성(미발송).
**Wizard#1**: 재조회를 안 해 draft_json에 채울 새 확정 데이터 없음 — 20차 초안 그대로 유효. 재조회 승인 시 채워질 5개 경로만 표로 사전 정리.

**남은 승인(개별, 포괄 승인 아님)**: ①보강된 어댑터로 CH1147184 재조회 1회(재시도 없음) ②속성비교 resolve_run() 실행(재조회 또는 사람의 실물·공식자료 근거 확보 후에만 가능, 현재 불가) ③온채널 공식 문의 발송(20차 초안 + 신규 문항) ④판매신청 NEEDS_REVIEW 검토 해제(문의 회신 후에만) ⑤Wizard#1 draft_json 저장(①②완료 후에만 의미 있음) ⑥실제 쿠팡 등록(①~⑤ 전부 + Coupang API/WING 연동 자체가 먼저 필요, 가장 먼 단계) — 전부 독립 승인 대상.
**"상품등록 차단 항목 집중 해소"까지이며 "V7 실사용 완료"·"실제 등록 완료"로 확대하지 않는다.**

---

**HOMEZ V7 — A/B/C 완료 후 상품등록 초안 완성 (2026-09-24, 20차). 19차 audit_logs 수정 시도 차단을 조사(Claude Code 하네스 안전장치, HOMEZ 제품 결함 아님, 0건 실행 확인) — 재정정은 추가 이벤트 방식으로 설계만(미실행). option_price(판매가) 필드를 seller=HOMEZ 역할 확정 사실과 연결해 "매입원가로 추정" 근거를 확립, 상품 기본가 필드 자체가 없고 옵션별 독립 단가만 있음을 스키마로 확인. 옵션 4종 가격이 2026-09-21 관측과 실제로 동일함을 재확인(변동 없음). **신규 발견**: 19차 C단계의 속성비교 부수효과가 BLOCKED 상태로 남아 있고, order_submission_service.py의 발주 게이트(상품코드 키)를 실제로 막는다(Wizard 게이트는 다른 식별자라 영향 없음) — 등록 차단 항목에 추가. Wizard#1 등록 초안 완성(원본 미저장), 내부 SKU 제안, 판매신청 확인 문의 초안 작성(미발송), 예시 판매가 계산(가정 명시).**

**결과(상세: `docs/HOMEZ_V7_CH1147184_LISTING_DRAFT_20260924.md`)**: 기준선 재확인 — 로컬/원격 HEAD `66a9e4f`(19차) 그대로, 다른 작업자 워크트리 보존. 코드 변경 없어 회귀 재실행 안 함.
**감사로그 조사**: 19차의 UPDATE 시도는 Bash 명령 전체가 하네스 분류기에 의해 실행 전 차단돼 DB에 전혀 도달하지 않았음을 재확인(부분 커밋 없음). HOMEZ 코드의 결함이 아니라 실행 절차 오류로 정리하고, 확대 감사하지 않았다. 재정정은 원본 수정이 아니라 같은 entity를 참조하는 신규 감사 이벤트 추가 방식으로 설계(승인 없이 미실행).
**가격 필드**: `docs/HOMEZ_ONCHANNEL_OPENAPI_FINDINGS_20260908.md`의 "seller=HOMEZ" 확정 사실과 공식 스키마(option_price=판매가, onch_price=공급가는 supplier 전용 엔드포인트)를 대조해 "옵션 가격 확인"(option_price)과 "공급가 미확인"의 차이를 정확히 설명. 상품 최상위에 기본가 필드가 없어 이 상품은 옵션별 독립 단가 구조임을 확정. disc_price/recom_cus_price/return_comment/contents/이미지 URL/gosi_info 등은 API엔 있었을 응답이지만 현재 어댑터 코드가 매핑하지 않아 캡처되지 않았음을 확인(코드 보강 후 재조회 1회로만 복구 가능, 이번엔 미실행).
**신규 발견(속성비교 BLOCKED)**: 19차 C단계 부수효과로 생성된 `product_attribute_comparison_runs#1`(product_identifier=CH1147184)이 실제로 `overall_status=BLOCKED`이며, `order_submission_service.py::submit_order()`의 `assert_attributes_confirmed_or_block(company_id, product_code)` 게이트를 구조적으로 막음을 코드 추적으로 확인 — Wizard#1의 `preflight()`는 다른 식별자(`candidate:3`)를 쓰므로 영향 없음. 관리자의 `resolve_run()`(각 항목 값 직접 선택) 없이는 해소되지 않음 — 등록 차단 항목에 신규 추가, 이번엔 해소하지 않음.
**Wizard#1 등록 초안**: 기존 진행점(current_step=CHANNELS, marketplace_listings#1 DRAFT/external_listing_id=None) 재확인, 새 Wizard 미생성. 옵션 4종 코드·구성수량·단가·고시정보 일부 정리, 이미지 이용권리·판매 제한·사용방법 등은 등록 차단 항목으로 유지. `supplier_option_links` 설계 원칙에 맞춰 내부 SKU(`PC3-CH1147184-{옵션코드}`) 제안(미저장), 미발급 쿠팡 식별자는 생성하지 않음.
**판매신청 확인**: NEEDS_REVIEW 유지, 공식 확인 방법(판매자센터 직접 확인/공급처 문의) 정리, 문의 초안 작성(발송 안 함).
**판매가**: 확정 마진·수수료 정책 없음(0/0) 재확인, 과거 스냅샷은 참고용일 뿐임을 명시하고 가정(m=30%, f=10.8%)을 명시한 예시 계산식과 옵션별 예시가 제시(세금·반품비·광고비 미포함 명시, 원본 가격 미변경).

**남은 승인(개별, 포괄 승인 아님)**: ①판매신청 상태 공식 확인(문의 발송 포함) ②속성비교 BLOCKED 해소(관리자 값 선택) ③어댑터 코드 보강 후 CH1147184 재조회 1회(disc_price 등 미캡처 필드 확보) ④자료 이용권리·판매 제한(판매자 자격) 조사 ⑤원본 Wizard#1 draft_json 저장(옵션·SKU 반영) ⑥실제 쿠팡 등록(13차 방어 적용 상태에서 1회) — 전부 독립 승인 대상.
**"A/B/C 완료 후 상품등록 초안 완성"까지이며 "V7 실사용 완료"·"실제 등록 완료"로 확대하지 않는다.**

---

**HOMEZ V7 — D2 실행: A/B/C 실제 적용 완료 (2026-09-24, 19차). 명시적 승인 확인 후 A(Migration 3건 적용)·B(CH1147184 NEEDS_REVIEW 기록)·C(상품 조회 1회) 전부 실제 실행·성공. 원본 homez.db 스키마 변경됨(해시 f3aafca1...→dbc6f966...), 기존 데이터 156개 테이블 전부 보존 확인. 옵션 4종 가격이 2026-09-21 과거 관측과 동일함을 실제 조회로 재확인(변동 없음). 검토 해제·판매신청·쿠팡 등록·실주문은 이번에도 미실행.**

**결과(상세: `docs/HOMEZ_V7_D2_ABC_EXECUTION_20260924.md`)**: 기준선 재확인 — 로컬/원격 HEAD `f09c398`(18차) 그대로, 다른 작업자 워크트리 3개 보존. 코드 변경 없어 회귀 재실행 안 함.
**A**: 백업(SQLite 백업 API, `Homez-Backups\v7_migration_18th_20260924\`) 생성·검증(157개 테이블 행수 일치) 후 미적용 3건을 `plan_pending()` 재확인(승인안과 완전 일치)하고서만 적용. 적용 후 integrity/FK/ORM 전부 통과, 기존 156개 테이블 데이터 무변경 확인(`schema_migrations`만 73→76으로 정상 증가), 새 테이블 2개(0행)·새 인덱스 1개 생성 확인, 재적용 안전한 no-op 확인.
**B**: `record_unconfirmed_prior_evidence()`로 CH1147184에 NEEDS_REVIEW 기록(신규 행 id=1, 사건시각 2026-09-14/기록시각 2026-09-24 분리, 외부 호출 0회, 감사로그 기록됨). 기록 중 콘솔 인코딩 문제로 생긴 로마자 표기를 운영 데이터(failure_detail)만 정정 — audit_logs 원문 수정은 시스템이 "감사 기록 변조"로 정확히 차단해 그대로 존중하고 중단.
**C**: 실행 직전 코드 재추적 중 승인안에 없던 부수 효과(product_attribute_comparison_runs 기록 1건)를 발견해 사용자에게 별도 확인받은 뒤에만 진행 — `lookup_product()` 1회 호출(재시도·페이지네이션 없음을 코드로 확인 후 실행), 옵션 4종 코드·가격이 18차 문서의 과거 관측과 완전히 동일함을 실제 응답으로 확인(가격 불변), 재고는 여전히 API 미제공, 배송비 필드(기본 3,000/제주 6,000/도서산간 8,000) 확인. 연결 verified_at 갱신, NEEDS_REVIEW는 그대로 유지됨을 재확인(조회 성공을 승인으로 해석하지 않음). 자격증명 원문 미노출 확인.

**남은 승인**: 판매신청 상태 공식 확인(문의 발송 포함, 별도 승인) → 검토 해제 실행 → Wizard#1 재개(SKU·구성수량·공급가·자료이용권리·판매제한 확정, 새 발주 승인 발급) → 판매가 산정(사용자 결정) → 실제 쿠팡 등록(별도 승인). 전부 개별 승인 대상, 이번에 포함하지 않음.
**"A/B/C 실제 적용 완료"까지이며 "V7 실사용 완료"로 확대하지 않는다.**
...
A 재확인: `plan_pending()`을 실 DB에 재실행해 미적용 3건이 기존 승인안과 파일명·순서·해시까지 완전히 일치함을 확인 — 차이 없으므로 미실행. B 재확인: company_id=1/connection_id=4(ONCHANNEL, sin9**, CONNECTED)/product_code=CH1147184 전부 변경 없음, 내부 행 0건 재확인 — 미실행. C: 연결 id=4·상품코드 재확인만, 실제 조회 미호출.
...
사본(14차에서 만든 Migration 3건 적용본 재사용, 원본 불변 재확인)에서 A+B 연결 검증: `record_unconfirmed_prior_evidence()`(검증된 서비스 경로)로 CH1147184에 NEEDS_REVIEW 기록(중복 호출도 안전, 새 행 생성 없이 같은 행 반환) → 완전한 재시작(새 engine/session/서비스 인스턴스, 가짜 Credential Store만 재사용) → 실제 발주 진입점(`submit_order()`)을 즉시-예외 발생 Fake Provider로 호출 → `ConflictException`으로 판매신청 POST 0회·발주 Provider 호출 0회 확인, 차단 사유·다음 확인 행동이 예외 메시지에 포함됨을 확인. 이 결과를 "원본 적용 후 검증"이라 부르지 않음 — 사본 검증일 뿐.
승인안 A(Migration 3건, 14차와 파일명·순서·해시 전부 동일 재확인)와 승인안 B(CH1147184 NEEDS_REVIEW 데이터 반영, Migration과 독립)를 분리해 하나로 제출. 과거 어떤 라운드에도 사용자의 실제 승인이 없었음을 명시(이전 보고서·승인안 표를 승인으로 해석하지 않음). 이후 실행 체인을 "기존 실행 체인 유지" 같은 포괄 표현 대신 8개 개별 항목(시험상품 선택/Wizard#1 보완/외부 조회/쿠팡 등록/매핑/시험 주문/발주·결제/반품·환불)으로 분리해 각각 대상·외부 호출 상한·DB 쓰기·재시도·중단 조건을 명시.
집중 테스트 174건(영향받은 6개 파일 고정 기준선 통합 실행) 전부 통과, 실패·오류 0건, 가드 로그 `GUARD_ACTIVE` 확인·`ATTEMPT_BLOCKED` 0건. `marketplace_listing` 도메인은 코드 변경 없어 재실행하지 않음. 실 `homez.db` SHA-256 불변 재확인.

**남은 승인(갱신, 중복 없음)**: 승인안 A(원본 Migration 3건 적용, 14차와 동일) / 승인안 B(CH1147184 `record_unconfirmed_prior_evidence()` 원본 반영, 검증된 서비스 경로로 실행) / 외부 확인 C(CH1147184 상품 상태 조회 1회, 승인 상태와 판매 상태 구분 유지) / 이후 실행 8개 항목(개별 승인, 포괄 승인 아님) — ①시험상품 최종 선택 ②Wizard#1 로컬 초안·옵션 보완 ③외부 상품·옵션 조회 ④실제 쿠팡 상품등록 ⑤HOMEZ 매핑 저장 ⑥정책상 허용된 시험 주문 ⑦금액·수량·배송지 확정 발주·결제 ⑧반품·환불 시험.
**"미확인 판매신청 실행 차단 코드 방어 완료 및 A/B 사본 연결 검증 완료"까지이며 "V7 실사용 완료"·"원본 DB 반영 완료"로 확대하지 않는다.**

---

**HOMEZ V7 — 13차 검증 증거 보완 및 Migration 3건 적용 준비 (2026-09-24, 14차). [15차에서 정정: "원본 데이터 보완 전에는 코드로 닫을 수 없는 경계"는 오류 — 코드가 과거 이력을 자동으로 알아낼 수 없는 것과 미확인 상태의 실행을 차단할 수 없는 것은 다르다. 상세: 위 15차 항목.] 가드 로그 증거 없이 주장했던 "ATTEMPT_BLOCKED 0건"을 철회하고 로그 근거로 재확인. 786/777 두 숫자를 실행 횟수 합계 vs 고정 기준선 회귀로 명확히 분리. 온채널 자동 재신청 공백을 쿠팡과 분리해 신규 테스트로 직접 증명. Migration 3건 사본 리허설 완료, 승인안 통합 갱신. 원본 DB는 여전히 미실행.**

**결과(상세: `docs/HOMEZ_V7_EVIDENCE_HARDENING_AND_MIGRATION_REHEARSAL_20260924.md`)**: 기준선 재확인 — 로컬/원격 HEAD `2787b1f`(13차 커밋) 그대로, 다른 작업자 변경 보존. 13차의 "7개 파일" 서술은 정확했음을 재확인(정정 대상은 12차 "6개"였고 13차 문서에서 이미 바로잡혀 있었다). 13차 문서 안에서 근거 없이 병기됐던 "777개 테스트"와 "786건 실행"을 분리: 786 = 실행 횟수 합계(9개 테스트가 개발 중간 확인+최종 배치에서 2회씩 실행된 것 포함, 중복 제거 안 함), 777 = 최종 고정 기준선 회귀(183+94+500, 파일 기준 상호 배타적, 중복 제거됨). 13차의 183건·500건 배치는 저장소 전체 회귀(총 322개 테스트 파일)가 아니라 `MarketplaceSubmission` 관련 도메인 파일 28개 한정 회귀였음을 명시.
13차가 로그 경로 미지정으로 근거 없이 주장했던 "ATTEMPT_BLOCKED 0건"을 철회 — 가드 자체 시험(`test_regression_guard_selftest`, 8/8)으로 로깅 메커니즘이 실제로 의도된 차단을 기록함을 먼저 양성 대조로 확인한 뒤, 증거가 없던 500건 배치만 고유 로그 경로로 재실행(시작·종료 시각·기준선 커밋·종료코드 보존) — `GUARD_ACTIVE` 확인, `ATTEMPT_BLOCKED` 0건(이번엔 실제 근거 있음). 786건·저장소 전체는 이유 없이 반복하지 않았다.
쿠팡 MarketplaceSubmission 방어와 온채널 판매신청 재시도 방어를 명시적으로 분리: "쿠팡 Submission 없음 → 온채널 위험 없음" 추론을 쓰지 않고, CH1147184류 상태(과거 접수 증거 있음/내부 행 0건/NEEDS_REVIEW 미기록)를 신규 격리 테스트로 실제 발주 진입점(`submit_order()`)에서 재현 — 판매신청 POST 1회·발주 Provider 호출 1회가 실제로 나감을 확인(결함이 아니라 원본 데이터 보완 전까지 코드로 닫을 수 없는 설계상 경계). NEEDS_REVIEW가 이미 기록된 뒤에는 완전한 재시작(새 세션·새 서비스 인스턴스) 후에도 방어가 유지됨을 별도 테스트로 확인.
Migration 3건(`20260918_00_...`, `20260921_00_...`, `20260924_00_...`)을 SQLite 백업 API로 만든 일관된 사본에 순서대로 적용해 리허설: 새 인덱스 조건 기준 충돌 데이터 0건 확인 후 적용, integrity/FK/157개 테이블 행 수 대조/ORM 호환/재적용 방지 전부 통과, 적용된 실제 스키마에서 서로 다른 키의 동시 등록 시도가 `IntegrityError`로 차단됨을 별도 확인. 원본 승인안을 하나로 통합 갱신(대상 DB·파일명·순서·해시·백업/복구 조건), CH1147184 `NEEDS_REVIEW` 데이터 반영은 Migration과 별도 항목으로 유지.

**남은 승인(갱신, 중복 없음)**: 승인안 A(원본 Migration 3건 적용, 백업 후) / 승인안 B(CH1147184 `record_unconfirmed_prior_evidence()` 원본 반영, 데이터 변경) / 승인안 C(CH1147184 상품 상태 조회 1회) / 승인안 D(시험상품 확정·쿠팡 문의·Wizard#1 재개·실제 등록·매핑·실주문·발주·결제·환불 — 기존 유지).
**"13차 증거 공백 보완 및 Migration 리허설 완료"까지이며 "V7 실사용 완료"·"원본 DB 반영 완료"로 확대하지 않는다.**
...
근거를 외부 사실/내부 기록/코드 차단 사유로 분리했다: ①외부(온채널) 현재 상태는 9일간 재조회 없어 미확인 ②내부(`purchase_sales_application_attempts`) 행 0건은 재확인된 사실 ③내부 차단은 "재신청 자동 유발"이며, `apply_for_sale()` 코드 자체가 "이미 신청된 상품 재신청 시 온채널 응답이 확정되지 않았다"고 이미 명시(409 추정, 공식 확정 아님) — 그래서 재신청은 이번에도 실행하지 않았다.
Wizard#1을 필드 수준까지 재확인: `product_candidate_id=3`(→`candidate_key`에 `ONCHANNEL:CH1147184` 명시적으로 저장 확인), 4단계(`CHANNELS`)에서 정지, `draft_json.options=[]`(옵션 미입력), `materialized_listing_ids_json=[]`(외부 리스팅 미생성) — 새 Wizard 생성이 아니라 기존 Wizard#1 재개가 맞는 다음 행동임을 확인. `PurchaseOrderApproval#1`은 재사용 금지 재확인(원본 미수정).
공식 조회 방법(새 엔드포인트 추정 아님): 기존 `GET seller/product/{code}`(`lookup_product()`)의 `status` enum(1=판매전~5=품절)이 판매신청 정황의 유일한 간접 신호임을 확인, 1회 조회 계획만 수립(D3 §2-3 기존 예산 내, 미실행). 내부 기록 보완은 설계만 제시(신규 상태값 제안, 구현·원본 반영 없음).
...
"sin9**"(id=4)이 둘 다 CONNECTED로 표시된다고 보고했고, 이어서
...
- HOMEZ 로그인(실제 사용자 세션 탭에서 재로그인) → "매입 운영
  ...
  인증 확인"으로 표시되는지 확인.
- id=4 "sin9**"를 유지할지 비활성화할지 — 이 세션이 만든 연결이
  아니라 임의로 처리하지 않았다.
...
결과, 실제로는 id=4 "sin9**"에 등록됐다(레거시 중복 버튼 혼동
...
붙인 표시 이름, 예: "sin9**")은 다른 개념임을 화면에서 구분할 수
있게 했다:
