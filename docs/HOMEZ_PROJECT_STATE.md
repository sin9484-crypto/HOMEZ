...
**기준선·최종 회귀**: HEAD `3703273`(원격 일치). 고정 기준선 전체 회귀 1회(외부 접속 차단 가드
적용): **4,563건 실행 / 실패 1 / 오류 0 / skip 7 / exit 1 — 전체 통과가 아니다.** 실패 1건은
상품후보 도메인의 2스레드 동시성 테스트(`test_concurrent_verify_info_only_one_succeeds`)로
`purchase_task`와 무관하며, 이전 기준선 `dec78db`에서도 40회 중 2~3회 실패해 기존 간헐 실패
(약 5%)로 확인했다(테스트 수정·완화 없음, 동일 코드 전체 회귀 반복 없음). 회귀 중 외부 접속
차단 시도 0건, `homez.db` SHA-256·크기·mtime 전후 동일(관측 사실). 이전 집중 회귀(443건+32건)와
합산하지 않는다. 미호출 예약 만료 함수의 향후 보호 조건은 감사 문서 33절에 기록(현재 실행 결함
아님). 상세: 감사 문서 33절.

**사용자 상품 3종 비교(D3 준비서 2-1절)**: ① CH1147184 샴푸/바디워시(정보 정확성 최고·액체
리스크), ② CH5250918 이어폰 후크(배송기간 "1일~2121일"·KC "[전기용품]안전인증" 표기 오류
가능성, 입점 1개월), ③ CH4922462 아이시스 생수 20병(무게 약 10kg 추정, 수량별 배송비 1개마다
부과, 식품 고시 대부분 "상세정보 별도표기"). 공급가·옵션·재고는 로그인 필요로 전부 미확인.
새로 조사한 세탁망·책꽂이·거름망은 추가 후보로만 구분. 시험상품은 확정하지 않았다(추천 근거만
제시, 최종 선택은 사용자).
