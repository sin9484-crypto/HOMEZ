...
File : tests/test_account_registration_migration.py

회원가입/승인 워크플로(user_registration_requests, invitation_codes)
Migration 정적/적용 검증 + Model 자동 드리프트 감지 + 중간 실패
rollback 검증. 표준 라이브러리 unittest + sqlite3만 사용. homez.db는
사용하지 않고 임시 SQLite 파일 DB만 사용한다.
=========================================================
"""
...
EXPECTED_TABLES = {"user_registration_requests", "invitation_codes"}
MODEL_TABLE_ORDER = [UserRegistrationRequest, InvitationCode]
...
class AccountRegistrationMigrationStaticTestCase(unittest.TestCase):
...
class AccountRegistrationMigrationApplyTestCase(unittest.TestCase):
    ...
    def test_mid_migration_failure_rolls_back(self):
        ...
        conn = sqlite3.connect(self.db_path)
        try:
            conn.execute("CREATE TABLE user_registration_requests (conflict_marker INTEGER)")
            conn.commit()

            with self.assertRaises(sqlite3.OperationalError) as ctx:
                conn.executescript(self.migration_sql)

            self.assertIn("user_registration_requests", str(ctx.exception))
            self.assertIn("already exists", str(ctx.exception))

            conn.rollback()

            columns = {
                row[1] for row in conn.execute("PRAGMA table_info(user_registration_requests)")
            }
            ...
            self.assertNotIn("invitation_codes", tables)
        finally:
            conn.close()
