...
File : tests/test_auth_session_migration.py

HOMEZ Desktop 로그인 흐름 — auth_sessions Migration 정적/적용 검증 +
Model 자동 드리프트 감지 + 중간 실패 rollback 검증. 표준 라이브러리
unittest + sqlite3만 사용. homez.db는 사용하지 않고 임시 SQLite 파일
DB만 사용한다.
=========================================================
"""
...
class AuthSessionMigrationStaticTestCase(unittest.TestCase):
...
class AuthSessionMigrationApplyTestCase(unittest.TestCase):
    ...
    def test_mid_migration_failure_rolls_back(self):
        """
        auth_sessions와 이름이 겹치는 충돌용 테이블을 미리 만들어두면
        ...
        conn = sqlite3.connect(self.db_path)
        try:
            conn.execute("CREATE TABLE auth_sessions (conflict_marker INTEGER)")
            conn.commit()

            with self.assertRaises(sqlite3.OperationalError) as ctx:
                conn.executescript(self.migration_sql)

            self.assertIn("auth_sessions", str(ctx.exception))
            self.assertIn("already exists", str(ctx.exception))

            conn.rollback()

            columns = {
                row[1] for row in conn.execute("PRAGMA table_info(auth_sessions)")
            }
