...
class NoSingleConditionAloneOpensRealOrderPathTestCase(OrderSubmissionServiceTestCaseBase):
    ...
    def test_product_lookup_success_alone_does_not_open_order_path(self):
        """이 조건이 핵심 사례다 — 게이트A는 (설계대로) 정말
        통과한다. 그래도 실제 발주가 열리지 않는 것은 게이트B(계약
        미확인)가 독립적으로 막기 때문이다."""
        ...
        self.assertEqual(
            ...
            "(사용자 확정 사항) — 이 테스트가 증명하는 것은 그래도 실제"
            "발주까지는 도달하지 않는다는 점이다.",
        )

        # 게이트A는 (설계대로) 통과한다는 것을 계약이 확인됐다고
        # 가정했을 때만 별도로 증명한다(실제 기본값에서는 게이트B가
        # 먼저 막으므로 이 사실 자체를 실제 설정으로는 관찰할 수 없다).
        with mock.patch(
            "app.domains.purchase_task.channel_connection_service."
            "is_onchannel_order_contract_fully_confirmed",
            return_value=True,
        ...
        # 실제 기본값(계약 미확인)에서는 게이트B가 독립적으로 막는다.
        with self.assertRaises(ConflictException):
            self.connection_service.verify_connection_ready_for_order_submission(
                connection.id, self.company_a.id,
            )
    ...
    def test_order_lookup_success_alone_does_not_open_order_path(self):
